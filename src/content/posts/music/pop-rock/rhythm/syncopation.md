...
sourceLink: https://openmusictheory.github.io/syncopation.html
comment: true
draft: false
---
...
*切分*发生在原本通常落在强拍或拍的强位上的节奏音型，改为落在弱拍或拍的弱位上之时。大多数流行／摇滚歌曲都是切分节奏与“平直”节奏的混合。切分节奏通常容易唱，因为它们往往比平直节奏更贴合语言。然而，它们比平直节奏更难视唱、听写或记录。

## Straight syncopations｜平直切分
...
在当代流行／摇滚音乐中，切分通常的做法是：取一串时值相等的音，把第一个音的时值减半，并把其余各音提前相应的半值。
...
例如，一串四个都落在拍点上发声的四分音符，可以用这种方式变形：把第一个音变为八分音符，并让其后每一个四分音符都提前一个八分音符发声——全部落在*弱位*（offbeats）上。
...
这一过程可以发生在任何节拍层级上。如果这串“平直”音的时值为两拍，则切分的做法是把第一个音改为单拍，并把其余每个音提前一拍。如果平直音的时值像上图那样为一拍，则切分要靠一个分拍（简单节拍中即半拍）来实现。如果这些平直音各自是一个分拍，则切分要通过把一个细分单位提前来实现。切分的单位（第一个音的时值，以及施加于后续各音的提前量）总是平直音时值的一半。所有这些切分在当代流行／摇滚音乐中都相当常见。
...
按照约定，当我们取一串时值各为一拍的音并将它们提前半拍时，我们称之为*拍级切分*。当我们取一串时值各为一个分拍的音并将它们提前一个细分单位时，我们称之为*分拍级切分*（division-level syncopation）。

*Beat-level syncopation.*

*拍级切分。*

[![四分音符转为八分音符](/media/music/graphics/syncopation/quarterToEighth.png)](/media/music/graphics/syncopation/quarterToEighth.png)

*Division-level syncopation.*

*分拍级切分。*

[![八分音符转为十六分音符](/media/music/graphics/syncopation/eighthToSixteenth.png)](/media/music/graphics/syncopation/eighthToSixteenth.png)

(Note the use of ties to make each beat clear. Always do this in order to make it easy to read.)

（注意连音线（ties）的使用，以使每一拍清晰可辨。务必这样做，以便于读谱。）

## Transcribing straight syncopations｜记录平直切分
...
平直切分节奏很容易通过频繁出现的弱位节奏来辨认。例如，如果你在听一首歌时指挥或拍打计数脉冲，那么连续几个音都出现在你的拍打或指挥的拍之间，而没有任何音与计数脉冲同时发声，就表明存在切分。
...
一旦辨出一段切分——它可能只涉及两三个音——就要判断切分发生在哪一个节拍层级上。例如，在简单节拍中，如果没有任何音直接落在计数脉冲的拍点上，而每一拍之间有且只有一个音发声，则切分发生在拍级上。如果没有任何音直接落在计数脉冲的拍点上，而每一拍之间有*两个音*发声，就一边拍打*分拍*一边再听这一段。如果没有任何音直接落在分拍的拍点上，而每两次拍打之间有一个音发声，则切分发生在分拍级上。
...
一旦确定了切分发生的节拍层级，就要确定提前量的时值。如果切分发生在拍级（每个计数脉冲拍之间有一个音发声），则切分的值为一个分拍：每个时值为一拍的音都被提前一个分拍。如果切分发生在分拍级，则切分的值为一个细分：每个时值为一个分拍的音都被提前一个细分单位。
...
最后，确定这个切分音型如何开始。这个弱位音型是直接从弱位开始吗？还是像上例那样以两个紧接的快速音开始——先是一个落在拍点上的短音，随后是较长切分音中的第一个音？
...
一旦你确定了切分的层级、提前量的时值，以及该音型是否以一个被截短的拍点音开始，这个节奏音型就应该容易记谱了。不过，如果你仍然感到困难，可以试着借助歌词的音节与歌词的重音模式，来帮你追踪各个音以及它们哪些在拍点上、哪些不在拍点上。在记谱节奏之前先把歌词写下来会大有帮助。
...
流行／摇滚中另一种常见的节奏音型是把一拍（或两拍）分为三个几乎相等的小组。例如，把一个二分音符分为两个附点八分音符与一个八分音符（3+3+2）。由于这一音型近似三连音，同时又保持着以 2、4、8 等对拍作简单划分的方式，我们可以称之为*伪三连音*。

![伪三连音（整小节）](/media/music/graphics/syncopation/fakeTripletsBar.png)

![伪三连音（半小节）](/media/music/graphics/syncopation/fakeTripletsHalfBar.png)
...
在大多数流行／摇滚体裁中，伪三连音比“真正的”三连音更为常见，但两者都会出现，因此要注意区分二者。Jump, Little Children 的《Cathedrals》同时包含了二者的例子（以及一些平直切分），是练习演奏与辨认伪三连音和真三连音的绝佳例子。
...
虽然伪三连音最常以 3+3+2 的分组出现，3+2+3 与 2+3+3 也是可能的。

A related pattern is what we might call *fake sextuplets*. Here the 3+3+2 pattern is doubled, resulting in 3+3+3+3+2+2.

一个相关的音型可以称为*伪六连音*。这里 3+3+2 音型被加倍，形成 3+3+3+3+2+2。

![伪六连音](/media/music/graphics/syncopation/fakeSextuplets.png)
...
在 U2 的《Electric Co.》的开头，吉他演奏按 3+3+3+3+2+2 分组的细分（十六分音符），而底鼓在踩镲演奏平直细分的同时演奏平直拍（四分音符）。

<iframe style="border-radius:12px" src="https://open.spotify.com/embed/track/7sSdgidO7APGUdad4Ssw0L" width="100%" height="152" frameborder="0" allow="autoplay; clipboard-write; encrypted-media; fullscreen; picture-in-picture" loading="lazy"></iframe>

## Demo｜演示

The following video walks through the process of transcribing straight syncopations in the song “Shh” by Frou Frou.

下面的视频逐步讲解为 Frou Frou 的歌曲《Shh》记录平直切分的过程。

<iframe src="http://player.vimeo.com/video/52491312?badge=0" width="100%" height="400" frameborder="0" allow="autoplay; fullscreen; picture-in-picture" allowfullscreen></iframe>
