---
title: "Analytical notation｜分析记谱法"
published: 2022-04-12
description: "流行摇滚曲式分析的记谱符号与标注规范。"
cover: /media/music/covers/gramophone.jpg
coverInContent: false
pinned: false
tags: [流行摇滚, 曲式]
category: [音乐理论, 流行与摇滚]
author: Open Music Theory
licenseName: CC BY-SA 4.0
licenseUrl: https://creativecommons.org/licenses/by-sa/4.0/
sourceLink: https://openmusictheory.github.io/popRockForm-notation
comment: true
draft: false
---

> **来源**：本文选自 [Open Music Theory](https://openmusictheory.github.io/popRockForm-notation)，由 Hybrid Pedagogy Publishing 出版，采用 [CC BY-SA 4.0](https://creativecommons.org/licenses/by-sa/4.0/) 许可。
>
> **中文名**：分析记谱法　|　**归档说明**：中英对照，每段英文原文后附中文译文，英文原文未作删减；图片与音频已本地化，站内链接已改为本站链接。

Following are notational conventions for analyses of musical form.

以下是音乐曲式分析所使用的记谱规范。

## Capital letters｜大写字母

Modules are labeled with capital letters according to function. A module that functions as a strophe is labeled with an “A”; a module that functions as a bridge, “B”; etc.

模块按其功能以大写字母标记。承担诗节（strophe）功能的模块标记为“A”；承担桥段功能的标记为“B”；依此类推。

## Lower-case letters｜小写字母

Phrases are labeled with lower-case letters according to their musical content. If two phrases use more-or-less the same musical framework (harmony, melody, and rhythm), they receive the same letter. Letters are assigned in the same manner as poetic rhymes: the first phrase is *a* and any phrase that follows based on the same music is also *a* (primes are used for slight variations, such as new text or altered instrumentation); the next phrase with new musical material is b; and so on. These letters do *not* correspond to functions.

乐句按其音乐内容以小写字母标记。若两个乐句使用大体相同的音乐框架（和声、旋律与节奏），则赋予它们相同的字母。字母的指派方式与诗歌押韵相同：第一个乐句为 *a*，其后任何基于相同音乐的乐句也为 *a*（撇号用于标示细微变化，如新歌词或配器改动）；接下来具有新音乐材料的乐句为 b；依此类推。这些字母*不*与功能相对应。

The single exception to this convention is when phrases within a module demonstrate a sentential progression (*srdc*), in which case the first phrase (statement) is labeled *s*; restatement/response, *r*; departure, *d*; conclusion, *c*.

这一惯例的唯一例外是，当某一模块内部的乐句呈现乐句式进程（*srdc*）时，此时第一个乐句（陈述 statement）标记为 *s*；重述／应答（restatement/response）标记为 *r*；离题（departure）标记为 *d*；收束（conclusion）标记为 *c*。

## Full-sized numerals｜全尺寸数字

Full-sized numerals are attached to capital letters when there are two or more modules with the same function but different music. For example, if a song contains two different melodies that both function as verse themes, they are labeled “V1” (the one that appears first in the song) and “V2.”

当存在两个或更多功能相同但音乐不同的模块时，将全尺寸数字附在大写字母之后。例如，若一首歌包含两条不同的旋律，且二者都承担主歌主题的功能，则分别标记为“V1”（歌曲中先出现的那一个）与“V2”。

## Subscript numerals｜下标数字

Subscript numerals are attached to capital letters when there are two or more modules with the same function and music but different text. For example, if a song contains three verses, and they all have different lyrics but the same music, they are labeled “V1”, “V2”, and “V3”.

当存在两个或更多功能与音乐相同但歌词不同的模块时，将下标数字附在大写字母之后。例如，若一首歌包含三段主歌，它们歌词各异而音乐相同，则分别标记为“V1”“V2”“V3”。

## Timeline notation｜时间轴记谱

[Variations Audio Timeliner](http://variations.sourceforge.net/vat/) does not support subscripts. It is fine to use full-sized numerals for both purposes if making timelines in VAT.

[Variations Audio Timeliner](http://variations.sourceforge.net/vat/) 不支持下标。若在 VAT 中制作时间轴，用全尺寸数字同时承担这两种用途也无妨。

[*Back to pop/rock form overview.*](/posts/music/pop-rock/form/popRockForm/)

[*返回流行／摇滚曲式概览。*](/posts/music/pop-rock/form/popRockForm/)
