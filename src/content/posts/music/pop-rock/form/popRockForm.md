...
sourceLink: https://openmusictheory.github.io/popRockForm.html
comment: true
draft: false
---
...
试看 Carl Perkins 的《Blue Suede Shoes》。这首歌包含多个*模块*，它们都具有相同的基本底层音乐。尽管配器与歌词有所变化，从 0:19 开始的部分在旋律、和声与乐句结构上，与从 0:58、1:37 和 1:54 开始的部分相同——或至少极为相似。再仔细听一点，我们能在歌曲开头听到同样模式的一种相似但缩减的版本。甚至 0:41 与 1:21 的器乐段落也具有相同的底层模式，只是旋律换成了吉他独奏。整首歌就是这一基本模式、或其细微变化的重复，其范本出现在 0:19–0:41。凡是通篇重复同一基本多乐句单位的歌曲，称为*分节歌*（strophic songs）。这种曲式称为*分节歌形式*（strophic form，有时缩写为 AAA，因为同一基本材料 A 被反复使用），而被重复的基本单位称为*诗节*（strophe）。分节歌形式在早期摇滚（1950 至 1960 年代）中比在 1970 年代及其后更为常见。
...
以下是《Blue Suede Shoes》曲式的鸟瞰式草图，可在聆听时对照：
...
While “Blue Suede Shoes” is composed entirely of strophes, it is important to note that strophic songs can also contain [*auxiliary modules*](/posts/music/pop-rock/form/popRockForm-containers/) such as intros, outros, and codas. However, if a song has more than one main musical idea other than strophes and auxiliary modules, it is not strophic, but likely one of the following two forms.

尽管《Blue Suede Shoes》完全由诗节构成，但须注意分节歌也可以包含[*辅助模块*](/posts/music/pop-rock/form/popRockForm-containers/)，如前奏、尾奏与尾声。然而，若一首歌在诗节与辅助模块之外还含有不止一个主要音乐构思，它便不是分节歌，而很可能属于以下两种曲式之一。

## 32-bar song form (AABA)｜三十二小节歌曲形式（AABA）
...
另一种在早期摇滚中更常见的曲式结构是 AABA 形式，也称三十二小节歌曲形式，其名称源于使用这一结构的早期“黄金时代”歌曲的某些特征。
...
试看 The Beatles 的《I Want to Hold Your Hand》。经过简短的前奏之后，歌曲以两个诗节开始。然而，在《Blue Suede Shoes》接着出现器乐诗节之处，The Beatles 于 0:52 转向一个*桥段*。这个新段落通过与主诗节主题形成对比并暂时悬置它来积累张力，直至主题于 1:11 回归。请注意，这首歌以诗节开始并以诗节结束，而诗节包含了标题歌词。对许多人而言，它也是歌曲中更令人难忘的部分。因此，诗节仍是*主要*模块。但此时它有了一个*次要*模块来增添趣味与张力，即桥段。（此外还有一个辅助模块——前奏——帮助歌曲顺利起步。）

Here is a bird’s-eye-view sketch of the form of “I Want to Hold Your Hand”:

以下是《I Want to Hold Your Hand》曲式的鸟瞰式草图：

- 0:00 – Intro／0:00 – 前奏
- 0:08 – Strophe 1 (A1)／0:08 – 诗节 1（A1）
- 0:29 – Strophe 2 (A2)／0:29 – 诗节 2（A2）
- 0:51 – Bridge (B)／0:51 – 桥段（B）
- 1:11 – Strophe 3 (A3)／1:11 – 诗节 3（A3）
- 1:33 – Bridge (B)／1:33 – 桥段（B）
- 1:53 – Strophe 3 (A3)／1:53 – 诗节 3（A3）
...
*这首歌没有合法的、开放获取的可嵌入音频。不过，若你尚未拥有唱片，也很容易找到。*

“I Want to Hold Your Hand” is a typical AABA song in that it does not just have four modules, AABA. AABA songs almost always have a complete AABA [*cycle*](/posts/music/pop-rock/form/popRockForm-containers/), followed by either another complete AABA cycle, or an incomplete cycle (typically BA). Once the first AABA cycle is complete, there tend not to be any new lyrics, only repetition of the whole or the end of the main cycle.

《I Want to Hold Your Hand》是一首典型的 AABA 歌曲，其典型性在于它并不只有四个模块 AABA。AABA 歌曲几乎总是包含一个完整的 AABA [*循环*](/posts/music/pop-rock/form/popRockForm-containers/)，其后或接另一个完整的 AABA 循环，或接一个不完整的循环（通常为 BA）。一旦第一个 AABA 循环完成，通常便不再有新歌词，只是重复整个循环或其结尾部分。

The convention is to label cycles with curly brackets: {}. So the large-scale form of “I Want to Hold Your Hand” is

惯例是用花括号 {} 标记循环。因此《I Want to Hold Your Hand》的大型曲式为

> {AABA}{BA}

## Verse-chorus form (VC, VCB)｜主歌—副歌形式（VC、VCB）
...
作为三种主要曲式类型中的最后一种，*主歌—副歌形式*是一种适应性极强的歌曲形式，它在 1960 年代迅速取代了摇滚乐中的其他形式，并自此主导了这一体裁。与 AABA 形式一样，主歌—副歌形式含有多个*核心*（非辅助）模块。然而，在 AABA 歌曲中，标题歌词、最令人难忘的音乐以及主要叙事往往都出现在诗节中（诗节既开始*又*结束歌曲）；而在主歌—副歌形式中，这些特征被分配于主歌（次要模块，包含主要叙事文本，并开始歌曲）与副歌（主要模块，包含标题歌词、最令人难忘的旋律，并结束歌曲）之间。
...
试看 Bon Jovi 的歌曲《Livin’ on a Prayer》。在一段较长的前奏之后，第一个循环于 0:47 以主歌开始。随后在 1:18，一个*预副歌*提升能量与张力，进入 1:34 的副歌。经过一段简短的曲中前奏后，该循环从 1:54 起重复，并在 2:56 增加了一个*后副歌*。3:00 处的最后一个循环并不典型且被缩减，其后若接以结尾副歌的多次重复，其间以淡出结束全曲。
...
与《Livin’ on a Prayer》一样，主歌—副歌歌曲的曲式循环至少包含两个核心模块——主歌（V）与副歌（C），其中副歌模块为主要模块。循环中可能出现的其他模块承担预副歌（P）、桥段（B）与后副歌（Z）功能。
...
一个包含除 B 以外所有模块的完整循环应为 {VPCZ}。这四种功能总是按此顺序推进，尽管并非全部都需要出现。桥段模块较为灵活。若一首歌只有一个桥段模块，它通常只出现一次，其后是该曲最后的副歌，或最后的预副歌与副歌。桥段常常在最后一个循环中取代主歌和／或预副歌模块，而非作为额外元素出现。因此，融合全部五种核心模块类型的歌曲，很少会把五种都放在同一个循环中。
...
常见的不含桥段的循环包括 {VC}、{VPC}，偶尔（尤其是作为歌曲的第一个循环）还有 {VVC}，其中任何一种都可以在末尾加上 Z。含桥段的常见循环为 {BC} 与 {BPC}，两者均可在末尾加上 Z。

## Simple verse-chorus form｜简单主歌—副歌形式
...
*简单主歌—副歌形式*是 John Covach 创制的术语，指那些主歌底层和声进行与副歌底层和声进行相同的主歌—副歌形式歌曲。U2 的《With or Without You》即为典型一例。
...
HTML 链接：<https://open.spotify.com/track/5JGEAz15LkPoOtFHttDtVs>
Spotify URI：<spotify:track:5JGEAz15LkPoOtFHttDtVs>

## Super-simple verse-chorus form｜超简单主歌—副歌形式
...
*超简单主歌—副歌形式*是 [Jay Summach](http://gradworks.umi.com/35/25/3525244.html) 创制的术语（基于 Covach 的术语），指那些主歌与副歌的和声进行与旋律都相同的主歌—副歌形式歌曲（Summach，第 322 页）。

## Going into detail｜深入细节
...
以下各节将更详细地讨论这些大型结构及其构成成分结构。OMT 中的术语、概念、定义与记谱规范，或取自通行惯例；或取自 [Jason Summach](http://gradworks.umi.com/35/25/3525244.html)、John Covach、Walter Everett、Mark Spicer 或 Daniel Harrison 已发表或未发表的研究；或取自上述来源的某种组合。

- [Terminology and basic concepts｜术语与基本概念](/posts/music/pop-rock/form/popRockForm-terms/)
- [Formal containers and module structures｜形式容器与模块结构](/posts/music/pop-rock/form/popRockForm-containers/)
- [Formal functions｜形式功能](/posts/music/pop-rock/form/popRockForm-functions/)
- [Analytical notation｜分析记谱法](/posts/music/pop-rock/form/popRockForm-notation/)
