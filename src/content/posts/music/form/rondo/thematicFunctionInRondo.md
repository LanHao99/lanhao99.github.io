...
description: "回旋曲式中各段落的主题功能划分与辨识方法。"
...
sourceLink: https://openmusictheory.github.io/thematicFunctionInRondo.html
comment: true
draft: false
---
...
回旋曲式的特征在于*叠部*与*插部*的交替。叠部每一次返回都在主调上，并呈示相同的旋律／动机材料，而插部则在旋律与调性上形成对比。
...
*叠部*一般是简单、结构紧凑的主题，以主调上的完全终止（PAC）结束。在主题构建上，回旋曲式的叠部最常见的写法有两种：（1）作为简单的（通常结构紧凑、常为乐段式的）主题类型；（2）作为带再现的二部曲式。

If the refrain is a rounded binary, it is common for it to be abridged when returning later in the movement.

若叠部为带再现的二部曲式，则它在乐章后文返回时常常被删减。
...
与此相对，*插部*通常处于对比调，并具有对比性的旋律／动机材料。尽管并非总是如此，但最常见的情形是：与叠部形成对比的插部在构建上比叠部更复杂。为回到叠部制造戏剧性，是回旋曲式美学的重要部分，而与回旋曲式插部相联系的主题与和声上的复杂性，正是达成这一目标的重要一环。
...
**调性：** 内部主题与叠部形成调式或调性上的对比。多数时候，内部主题是"*minore*"（小调化），即从叠部到插部主音并未改变，但调式发生了改变。（在小调回旋曲式中，第一个插部可能是"*maggiore*"（大调化）。）
...
**曲式结构：** 内部主题通常是带再现的二部曲式，或其他的二部曲式类型。有时，这种二部结构在乐谱中用反复记号标示，不过偶尔反复也被"写出"。内部主题之后可以有*连接过渡段*（retransition）引向主调的属和弦，但这绝非必须。
...
*\*复杂情形：\*\*分析上的主要复杂之处在于，内部主题在曲式上常常被留作\*不完整*。应注意以下两种偏离：

1. Following a contrasting middle section, the recapitulation may not return. Thus, such an episode might be structured as AAB followed by a retransition.／在对比性中段之后，再现部可能不再返回。因此，这样的插部可能结构为 AAB，后接连接过渡段。
2. The recapitulation may return but not lead to a PAC, instead merging into a retransition section.／再现部可能返回，但并不导向完全终止（PAC），而是并入连接过渡段。

## Second-Theme Complex｜第二主题群
...
第二主题群类似于——但并不总是等同于——古典奏鸣曲式中的 TR、S、CL 与 RT 各区。在大调中，插部将转调到属调（V）。在小调中，大三度中音调（III）更为常见。
...
**曲式结构：** 我们先勾勒其原型形式，但如下文所示，你必须灵活地对待这一部分。在第一个叠部收束之后，一个过渡乐句（TR）通过中介和弦转调进入从属调，从而开启插部。随后是第二主题（S），以完全终止（PAC）确认新调。接着听到结束段（CL），并并入引回叠部的连接过渡段（RT）。
...
**复杂情形：** 尽管它类似奏鸣曲式呈示部的某些特征，回旋曲式的第二主题群却涉及多得多的可能性。例如，TR 与 CL 可能不出现，第二主题群也可能没有从属调上的完全终止（PAC）。此处的主题功能远比叠部中宽松。以下是主要的复杂情形：

1. **TR merges => into S:** Following the PAC that ended the refrain, a TR phrase begins and initiates a modulation. However, after modulating the same phrase confirms the subordinate key with a PAC. In this situation, the single phrase is understood to express both TR and S function. This is very much like the TR=>S merger that we find in the exposition of a Minuet/Trio form.／**TR 并合 => 进入 S：** 在结束叠部的完全终止（PAC）之后，一个 TR 乐句开始并启动转调。然而，在完成转调之后，同一乐句以完全终止（PAC）确认从属调。在此情形下，这个单一乐句被理解为同时表达 TR 与 S 功能。这极像我们在小步舞曲／三声中部曲式的呈示部中所见的 TR=>S 并合。
2. **No TR appear:** Following the PAC that ended the refrain, S appears with no intervening TR. In this case, a direct modulation has occurred.／**TR 不出现：** 在结束叠部的完全终止（PAC）之后，S 直接出现，中间没有 TR。此时发生的是直接转调。
3. **S does not lead to a PAC, but merges => into a retransition (RT)**: Typically, the episodes S theme will lead to a PAC that confirms the subordinate key. In this case, the cadence never materializes. Instead, the music more or less seamlessly becomes a retransition, S=>RT.／**S 不导向完全终止，而是并合 => 进入连接过渡段（RT）：** 通常情况下，插部的 S 主题会导向确认从属调的完全终止（PAC）。在此情形下，终止式从未真正出现。相反，音乐或多或少无缝地变为连接过渡段，即 S=>RT。
...
终止式结构对于判定主题的功能是*绝对必要的*。在回旋曲式的第二主题群中更是如此。以下若干分析提示围绕终止式结构展开，你应以此引导自己的分析。

1. S themes begin in a subordinate key, end with a PAC in a subordinate key, or both. That is, a phrase that ends with a PAC in the subordinate key expresses S function even if the phrase did not begin in the subordinate key, and vice versa.／S 主题起始于从属调、以从属调上的完全终止（PAC）结束，或二者兼具。也就是说，一个以从属调上的完全终止结束的乐句表达 S 功能，即使该乐句并非起始于从属调；反之亦然。
2. TR function is given to phrases that modulate. When TR doesn’t merge with S, it usually ends with a HC in the subordinate key.／TR 功能赋予发生转调的乐句。当 TR 不与 S 并合时，它通常以从属调上的半终止（HC）结束。
3. CL function follows a PAC in the subordinate key.／CL 功能接在从属调上的完全终止（PAC）之后。
4. RT usually contains a I:HC, optionally followed by “standing on the dominant.”／RT 通常包含一个 I:HC，其后可选地接"长时间停留在属和弦上"（standing on the dominant）。
