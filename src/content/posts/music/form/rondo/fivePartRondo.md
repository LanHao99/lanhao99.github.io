...
sourceLink: https://openmusictheory.github.io/fivePartRondo.html
comment: true
draft: false
---
...
五部回旋曲式包含三个叠部，由两个插部隔开：ABACA。（我们偶尔会用这类简写字母标记（A、B 等）来指称回旋曲式，但应养成把各段落标记为"叠部 1""叠部 2""插部 1"等的习惯，以免与更低层级的内嵌曲式产生歧义。）
...
五部回旋曲式的插部是内部主题或第二主题群。在插部 1 中出现第二主题群的情况，比在插部 2 中出现更为常见。叠部的再现常常被加花装饰或加以删减。
...
这一曲式类型所源出的更古老的 *rondeau*（回旋歌），可以有更多的叠部与插部。因此，在古典风格中偶尔也能见到七部乃至九部回旋曲式。
