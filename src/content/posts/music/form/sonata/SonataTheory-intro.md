...
sourceLink: https://openmusictheory.github.io/SonataTheory-intro.html
comment: true
draft: false
---
...
古典*奏鸣曲*是为独奏乐器、室内乐组合或管弦乐队而作的多乐章作品，其中至少有一个乐章采用*奏鸣曲式*。几乎总是如此：奏鸣曲的第一乐章采用奏鸣曲式。末乐章通常是热烈的*终曲*，可以采用若干不同的曲式。中间乐章（三乐章奏鸣曲的第二乐章；四乐章奏鸣曲的第二、第三乐章）通常是慢速乐章和／或舞曲乐章（小步舞曲或谐谑曲）。奏鸣曲中任何非舞曲性质的乐章都可以采用奏鸣曲式，但很少*全部*同时采用。通常只有第一乐章采用奏鸣曲式，或者第一乐章与另一个乐章。其余乐章采用其他标准曲式，如小步舞曲与三声中部、主题与变奏、回旋曲或奏鸣回旋曲。本单元中，我们将聚焦于奏鸣曲式，尤其是它们在器乐奏鸣曲第一乐章中的形态。
...
奏鸣曲式与*小型三部曲式*（具体说是*带再现的二部曲式*变体）共享许多结构特性。也就是说，奏鸣曲乐章的核心音乐可以理解为一个大型的 ABA’ 三部性模式。
...
然而，与小型三部曲式一样，奏鸣曲式乐章也具有二部性结构：第一个 A 部分构成乐章的第一个“半部”，B 与 A’ 部分共同构成第二个“半部”。这在早期的奏鸣曲乐章中最为明显，其中这两个部分各自被反复。这称为*双重反复*结构，在这一层面上看起来与带再现的二部性主题完全相同。

**||: A :||: B A’ :||**
which sounds like／听起来相当于
**AABA’BA’**

Later sonatas shed the repeat of the second reprise, only repeating the first reprise.

后来的奏鸣曲舍弃了第二部分的反复，只反复第一部分。
...
更晚的奏鸣曲往往把两组反复都舍弃，使该结构中的二部性方面（如果它仍存在的话）远比其三部性方面隐晦。
...
A 部分（或*模块*）具备我们所谓的*呈示*功能，A’ 部分具备*再现*功能。（这些名称与小型三部曲式的其他形式相同，如小步舞曲或主题与变奏乐章的主题。）B 部分具备我们所谓的*展开*功能。（这与小型三部曲式其他形式中的*对比中段*不同。）这些功能将在下文简要界定，并在其他资料中更详细地探讨。
...
如同小步舞曲那样，把 A 部分称为“呈示部”、B 部分称为“展开部”、A’ 部分称为“再现部”是常见且可接受的，只要我们始终保留曲式模块（即音乐段落本身）与曲式功能（即这些段落在更大音乐结构语境中*所做*的事）之间的概念区别。

## Formal functions in a sonata movement｜奏鸣曲乐章中的曲式功能

**Exposition function** involves three important features:

**呈示功能**涉及三个重要特征：

- establish the home-key tonality;／确立主调调性；
- move to and establish a secondary key by means of a strong cadence (PAC), called the *essential expositional closure* (EEC); and／借助一个强终止式（PAC）转向并确立副调，该终止式称为*必要呈示部收束*（EEC）；以及
- lay out the melodic themes and thematic cycle that will form the foundation for the rest of the movement.／铺陈旋律主题与主题循环，它们将构成乐章其余部分的基础。
...
我们可以把这看作一个提问，或至少是一个不完整的陈述：乐章所有主要的旋律材料都已呈现，但它结束在“错误”的调上。
...
**展开功能**通过以相同顺序循环同样的旋律、探索副调的不稳定性，凸显这一提问的开放性。这种探索可以通过以下任一方式实现：

- beginning in the secondary key, but modulating to another key before achieving a cadence;／从副调开始，但在抵达终止式之前转往另一调；
- employing non-functional harmonic progressions, such as harmonic sequences;／使用非功能性和声进行，如和声模进；
- frequent modulation; or／频繁转调；或
- multiple tonal goals other than the home key or secondary key.／除主调或副调之外的多个调性目标。

It then intensifies the need for an “answer” by progressing toward an *active dominant* (an HC or a major arrival on V or V7) in the home key.

随后它朝着主调中的*活跃属和弦*（一个 HC，或在 V 或 V7 上的主要到达）推进，从而加强了对“回答”的需要。
...
如果说呈示功能提出了一个“问题”，展开功能探索并随后强化了这个问题，那么**再现功能**则回答这个问题。再现功能的方式是最后一次行进过主题循环，但这一次把它引向令人满意的完成：主调中的一个 PAC，称为*必要奏鸣曲收束*（ESC）。

## Cadential goals｜终止目标
...
如上文所示，奏鸣曲式围绕若干重要的终止式而建立。它们既是曲式结构的路标，也是引向它们的音乐所指向的目标。我们将用表示调性（相对于主调）的罗马数字，后接冒号与终止式类型，来标记这些终止式。
...
与小型三部曲式一样，呈示部倾向于以*副调*中的 PAC 结束（大调乐章为 V:PAC，小调乐章为 III:PAC 或 v:PAC）。然而，这一终止式并非仅仅是一种常规，而是该曲式*必不可少*的组成部分。这个 PAC 称为*必要呈示部收束*，即 EEC。
...
再现部有一个与之对应的终止式，同样见于小型三部曲式：主调中的 PAC，它在主题上倾向于与 EEC 相对应。这个 I:PAC 称为*必要奏鸣曲收束*，即 ESC。
...
展开部与小型三部曲式中的对比中段一样，通常以主调中的 HC（I:HC）或主调中的*属到达*结束，从而为再现部的到来作准备。
...
通常，呈示部与再现部各自还有一个额外的终止目标，这是其他类小型三部曲式所没有的。这些目标分别出现在模块开头与终止目标（EEC 或 ESC）之间，而且往往——尽管并非总是——伴随旋律或和声运动的中断或停止。因此，这些中途的终止式各自称为*中间终止*（MC）。无论是 PAC 还是 HC，无论在主调还是副调，都可以充当中间终止。

## Defaults and deformations｜默认值与形变
...
奏鸣曲式并不是组织乐章的单一配方。它是一组规范，而这些规范在具体作品中几乎总会在某方面被违背，或至少被改动。因此，我们可以说个别作品与奏鸣曲式规范“处于对话之中”。也就是说，它们纳入了足够多的决定性要素，从而被视为“采用奏鸣曲式”，但同时带有足够多的独特要素，以致与规范之间产生某种张力。随着时间推移，古典作曲家所用的某些独特要素被后人采用，并成为下一代人的规范。因此，即便“典型的”奏鸣曲乐章，在不同时代与地域也不相同。
...
不过总体而言，对于奏鸣曲式的每个要素，我们都能识别出该要素的默认属性。有时会有多种可能性都算作规范。例如，古典奏鸣曲乐章的 MC 最常用的终止式是 V:HC，这称为*一级默认值*。第二常用的 MC 终止式是 I:HC，即*二级默认值*。当多种可能性都属规范、但其中一种比另一种更常见或更受偏爱时，我们便使用这套说法。
...
某个奏鸣曲乐章中偏离默认的属性称为*形变*。例如，以 II:IAC 作为 MC 完全不合古典奏鸣曲的典型。因此，我们会把它视为一种形变。这并不意味着 II:IAC 不能充当 MC（尽管我想不出这样的例子）。但它意味着，II:IAC 是作曲家有意识地违反规范的举动。因此，任何分析都应论及这一形变，并尝试解释其音乐与历史意义。
...
呈示—展开—再现的结构常常由*引子*和／或*尾声*加以框定。尽管这些模块中可以发生重要的音乐活动，它们仍在严格意义上的奏鸣曲之外。奏鸣曲的本质要素，以及那些在作品之间足够相似、以致我们能够很好地加以概括的要素，都出现在呈示部、展开部与再现部之中。尽管我们会在课堂分析中讨论引子与尾声材料，你也应在自己的分析中讨论它们，但这些模块的属性不在本网络资料的范围之内。
...
本站其余的奏鸣曲式资料大体采用参考手册的格式。它们不以典型教科书的方式逐一讲述细节，而是尽可能简洁地提供所述要素的定义性特征。这种简洁在你进行分析活动时快速查阅这些资料会大有帮助。然而，它也会迫使你在真实作品的语境中去与这些内容反复搏斗，才能真正理解它们。如果你只是单纯阅读这些资料，你不会理解奏鸣曲式。你必须在聆听与分析音乐的同时使用它们，才能吸收这些信息，并发展出将其在音乐上加以运用所需的技能。
