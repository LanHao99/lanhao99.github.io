...
sourceLink: https://openmusictheory.github.io/sonataThematicModules.html
comment: true
draft: false
---
...
构成典型二部式奏鸣曲呈示部的四个主题模块（P、TR、S 与 C），形成一种主题轮转；它会在再现部中再次出现，并常在展开部中被引用。

## Primary theme module (P)｜主部主题模块（P）
...
P 主题有若干功能：（1）确立主调；（2）呈示开启主题循环的主要旋律材料；（3）启动朝向 MC 与 EEC 的进行。
...
P 主题最常在和声上封闭，以 I:PAC 结束，或较不典型地以 I:HC 结束。（在较小的奏鸣曲中，P 可能是一个不以终止式结束的单个乐句（前句或呈示）。）当 P 主题以 PAC 结束，其后又跟随另一个同样以 PAC 结束的主题时，P 模块便包含一个“主部主题群”。主部主题群中的各主题用下标加以区分：P1 与 P2。

A P theme may exhibit any [standard theme type](/posts/music/form/theme-types/classicalThemes/) (sentence, period, hybrid, compound), though sentences are more common. Typically, a P theme is relatively “tight-knit” as compared to other thematic modules, containing a more straightforward presentation of tonic and less phrase deviations.

P 主题可以采用任何[标准主题类型](/posts/music/form/theme-types/classicalThemes/)（乐句式、乐段式、混合式、复合式），不过乐句式更为常见。与其他主题模块相比，P 主题通常相对“紧凑”，其主音呈示更为直接，乐句偏离也较少。

## Transition module (TR)｜连接模块（TR）
...
TR 模块的主要功能作用是推进到标志其结束的 MC。这既是和声的推进，也是修辞的推进，其特征是*能量积累*。对 TR 模块的分析应围绕 MC 以及作曲家如何趋近它展开。

### Harmonic motion｜和声推进
...
在奏鸣曲中，连接部可以是“转调的”或“不转调的”。不转调的 TR 引向 I:HC MC，而转调的 TR 引向 V（或 III）:HC MC。

### Melodic/motivic characteristics｜旋律／动机特征

We will follow Hepokoski & Darcy’s practice of locating the beginning of TR at the start of a phrase. In general, once you hear TR function clearly projected, track back to the beginning of that phrase and label it the beginning of TR. Unlike primary themes, transitions are much “looser” thematically. This module of the sonata is often associated with phrase [expansions](/posts/music/form/phrase-rhythm/internalExpansions/) and compression. More generally, anything that can be associated with [*continuation function*](/posts/music/form/theme-types/sentence/) fits transition function, as well: fragmentation, liquidation, acceleration of melodic or harmonic rhythm, etc.

我们将遵循赫普科夫斯基与达西的做法，把 TR 的开头定在一个乐句的起点。一般而言，一旦你清楚听到 TR 功能的显现，就回溯到该乐句的开头，并把它标记为 TR 的开始。与主部主题不同，连接部在主题上“松散”得多。奏鸣曲的这一模块常与乐句[扩充](/posts/music/form/phrase-rhythm/internalExpansions/)和压缩相关联。更一般地说，任何可与[*继续功能*](/posts/music/form/theme-types/sentence/)相关联的手段都契合连接功能：分裂、溶解、旋律或和声节奏的加快等等。

Melodically, we classify transitions as (1) *independent*, or (2) *dissolving*.

在旋律上，我们把连接部分为（1）*独立的*，或（2）*溶解的*。

An *independent TR* begins with new thematic material. In other words, it is not P-based.

*独立的 TR* 以新的主题材料开始。换言之，它不以 P 为基础。
...
*溶解型 TR* 模块都取用 P 主题的某一部分，并随着模块积累能量、朝 MC 推进而将其溶解、蜕变或化解。

- In the case of a *dissolving* restatement, P ends and then seems to begin again. This restatement of P dissolves into TR function, and we can subsequently reinterpret the whole theme as a dissolving-restatement type of TR.／在*溶解型*重述的情况下，P 结束之后似乎又再度开始。这一 P 的重述溶解为 TR 功能，我们随后可以把整个主题重新解释为溶解—重述型的 TR。
- In a *dissolving* consequent, continuation, or hybrid, the P module will consist of the opening phrase of a theme, and the closing phrase of the theme will begin as usual but degenerate into TR.／在*溶解型*后句、继续段或混合式中，P 模块将由主题的开头乐句构成，而主题的结束乐句会照常开始，但蜕变为 TR。
- A *dissolving* P-codetta will introduce post-cadential material to reinforce the cadence at the end of P, and that post-cadential material will dissolve into TR rhetoric.／*溶解型*的 P—小尾声会引入终止后材料以强化 P 末尾的终止式，而该终止后材料会溶解为 TR 的修辞。
...
在下面这个出自贝多芬《G 小调钢琴奏鸣曲》Op. 49 No. 1 的例子中，一个 III:HC MC 清楚地出现在第 15 小节，并与结构性的 HC 重合。回溯来看，注意第 12 小节的继续性特征，它表明连接部必定始于第 9 小节。（记住，我们总是把连接部的开始定在一个乐句的*起点*。）主部主题与连接部都以同样的呈示开始，因此这是一个*转调的*、*溶解型*的连接部。作为听者，我们很可能要到第 12 小节才会意识到连接部是连接部——正是在那里，继续段开始显现出与 TR 相关联的特征。

**Beethoven, Piano Sonata in G minor, Op. 49, No. 1, i**

**贝多芬《G 小调钢琴奏鸣曲》Op. 49 No. 1，第一乐章**

[![连接部示例](/media/music/graphics/form/TR.png)](/media/music/graphics/form/TR.png)

<iframe style="border-radius:12px" src="https://open.spotify.com/embed/track/5NSEaoEaSNdD3XQM54e0C6" width="100%" height="152" frameborder="0" allow="autoplay; clipboard-write; encrypted-media; fullscreen; picture-in-picture" loading="lazy"></iframe>

## Subordinate theme module (S)｜副部主题模块（S）
...
*S* 位于奏鸣曲的副调，其主要功能是引向该调中的一个 PAC——即*必要呈示部收束（EEC）*。由于它与这一核心和声事件（以及再现部中与之对应的终止式 ESC）的关系，S 模块在奏鸣曲式乐章中极为重要且引人关注。赫普科夫斯基与达西甚至说：“S 中发生的事情使奏鸣曲成为奏鸣曲”（第 117 页）。由于 EEC 具有如此重要的结构意义，它的到来常常被大幅延迟并戏剧化。
...
在主题上，S 主题比 P 主题松散得多。典型的做法是出现扩展的继续段与被扩充的终止性段落，所有这些都以戏剧化地呈现最终抵达 EEC 为目的。
...
需要注意的是，新的旋律材料*并不是* S 的必备条件。事实上，许多奏鸣曲——尤其是海顿所作的那些——其 S 主题与 P 主题相似。（这类作品称为单主题奏鸣曲。）

## Closing module (C)｜结束模块（C）
...
C 的决定性特征是：它接在 EEC 之后，并且它不是 S。C 模块可以呈示全新的主题材料，也可以借用 P 或 TR 的材料。按定义，它不能以 S 为基础，因为那将是 S 的继续。（请记住，EEC 之后必须接新材料，否则 S 模块仍在继续，而 EEC 尚未抵达。）
...
C 模块总是处于副调。它是终止后的，呈示部的和声目标已然抵达。如果 C 去了别的地方，它就不是 C。

## Retransition (RT)｜回归过渡（RT）
...
回归过渡类似于流行／摇滚或布鲁斯音乐中的*回头段*。它是主调中的一个属和弦或属到达，为呈示部反复开头处对主调的回归作准备。RT 与回头段的区别在于，RT 出现在一次转调之后。当 RT 跟随 V 的副调时，它通过重复、旋律音型化，或添加和弦七音，把 V 调中的 I 变为指向 I 调的 V。
