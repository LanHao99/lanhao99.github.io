...
sourceLink: https://openmusictheory.github.io/schemataContinuationPatterns
comment: true
draft: false
---
...
以下三种程式最早由十八世纪理论家 Joseph Riepel 描述。它们是[“延续型”](http://www.mtosmt.org/issues/mto.05.11.2/mto.05.11.2.eckert.html)，常出现在小步舞曲第一个双纵线之后。“Fonte”与“Monte”都是模进性的，“Ponte”则是一种延长某一和声的方式。
...
*Fonte*（意大利语，意为"泉"或"井"——联想*向下行*）是小型三部曲式*对比中段*开头的常见程式。换言之，它接在小步舞曲、小步舞曲三声中部或带再现二部主题的双纵线之后。Fonte 是一种*模型／模进*程式：一个两小节的样式立即比原样低一级重复。

[![fonte](/media/music/graphics/form/fonte.png)](/media/music/graphics/form/fonte.png)
...
在和声上，第一个两小节单位（即*模型*）包含两个和弦，每小节一个：一个应用属和弦，以及该应用属和弦所指向的被主音化和弦。Fonte 模型最常见的和弦样式是*主调的* **D7/II T1/II**，其中 **D7** 为六和弦，**T1** 为五和弦。（其他"转位"也可能出现，如 **D4/II T3/II**。）当模型把 **D7/II II** 展开写足时，模进会将其向下移到主调：*主调的* **D7 T1**。

As an example, the functional-bass analysis of a typical Fonte in a small ternary whose home key is G major looks like:

例如，一个主调为 G 大的小型三部曲式中典型 Fonte 的功能低音分析如下：

[![Haydn27fonteAnnotated](/media/music/graphics/form/Haydn27fonteAnnotated.png)](/media/music/graphics/form/Haydn27fonteAnnotated.png)
...
注意那些非终止性的进行 **D7–T1**。通常这类进行需要被解释为延长某一调性功能（即主功能），而在此处这样解释会很困难。程式常常包含这类进行。只需逐个分析和弦并标记该程式，而不必试图把这些进行解释为延长性的。（它们确实不是延长性的。）

A common model for a minuet containing a Fonte is as follows:

包含 Fonte 的小步舞曲的一种常见模式如下：

|  | : EXPOSITION ending with V:PAC : ／ : 呈示部，以 V:PAC 结束 : |  | : Fonte - phrase ending with I:HC – RECAPITULATION : ／ : Fonte —— 乐句以 I:HC 结束 —— 再现部 : |  |
| --- | --- | --- | --- | --- |
...
Fonte 是作曲家从属调（大调小步舞曲的呈示部正是在双纵线之前终止于该调）过渡到主调的快捷方式。它之后通常接一个停留在或进行到主调属和弦的乐句。该乐句末尾的半终止或*属音到达*，将为回到主调上的开头材料——即小步舞曲或小型三部曲式的*再现部*——做好准备。
...
*Monte*（意大利语，意为"山"——联想*向上行*）的功能与 Fonte 类似。它通常作为小步舞曲或其他小型三部曲式*对比中段*的一部分出现，是一种模型／模进程式，并且涉及一个应用和弦解决到被主音化和弦——典型样式为 **D7 T1**。差别在于：Fonte 是*向下*行（**D7/II T1/II D7 T1**），而 Monte 是*向上*行（**D7/IV T1/IV D7/V T1/V**）。此外，Fonte 几乎一律为四小节长（一个模型后接一次移调重复），而 Monte 有时扩展到六小节或更长（一个模型后接*一次或多次*移调重复）。

[![monte](/media/music/graphics/form/monte.png)](/media/music/graphics/form/monte.png)
...
*Ponte*（意大利语，意为"桥"）是小步舞曲*对比中段*的另一常见程式。与 Fonte 和 Monte 不同，Ponte 不必是模型／模进程式。它造成的是*延迟*而非*运动*。Ponte 的功能通常类似 Caplin 所称的*长时间停留在属和弦上*。大调小步舞曲的呈示部将以主调属调上的完全终止（PAC）结束。当 Ponte 接在该终止式之后时，它保持在那个 **T1/V** 上，在旋律上加剧紧张度，并常常给和弦加上七音（使其成为主调的 D5）。建立在 Ponte 之上的段落没有终止式，因为其中没有和声进行，而是以主调上一个被强调的属和弦结束，Caplin 称之为*属音到达*而非半终止。这一属音到达为回到主调以及小步舞曲再现处开头的那个基本乐思做好准备。
