---
title: "Composing a second-species counterpoint｜第二类对位写作"
published: 2022-01-16
description: "二对一音符的严格二声部写作规则与检查清单。"
cover: /media/music/covers/gramophone.jpg
coverInContent: false
pinned: false
tags: [对位法, 种类对位]
category: [音乐理论, 声部进行与范例写作]
author: Open Music Theory
licenseName: CC BY-SA 4.0
licenseUrl: https://creativecommons.org/licenses/by-sa/4.0/
sourceLink: https://openmusictheory.github.io/secondSpecies.html
comment: true
draft: false
---

> **来源**：本文选自 [Open Music Theory](https://openmusictheory.github.io/secondSpecies.html)，由 Hybrid Pedagogy Publishing 出版，采用 [CC BY-SA 4.0](https://creativecommons.org/licenses/by-sa/4.0/) 许可。
>
> **中文名**：第二类对位写作　|　**归档说明**：中英对照，每段英文原文后附中文译文，英文原文未作删减；图片与音频已本地化，站内链接已改为本站链接。
...
在第二类对位中，对位线条以二分音符运动，与全音符写成的固定旋律相对。这种 2:1 的节奏比例带来两个新的“基础音乐问题”——一个是节拍的，一个是和声的：*强拍*与*弱拍*之间的区分，以及*经过音*不协和的引入。

<iframe src="https://trinket.io/embed/music/bd3da422f4" width="100%" height="300" frameborder="0" allowfullscreen></iframe>
...
在第二类对位中引入和声不协和，会增加音乐织体的变化。然而，它也带来一种张力，必须用协和来平衡以促进调性融合，并且需要仔细留意，才能在进入和离开不协和时保持平滑。

## The counterpoint line｜对位线条
...
与第一类对位一样，对位线条应便于歌唱、形态良好、只有一个高潮，并以级进为主（为求变化可有一些小跳和偶尔的大跳）。然而，由于第一类对位的音符太少，为在练习的其他方面保持平滑，旋律常常不得不使用小跳。在第二类对位中，音符数量增加，加上使用不协和所带来的额外自由，使得以级进移动而不引发其他音乐问题变得更容易。因此，第二类对位比第一类*更加以级进为主导*。
...
如果对位必须跳进，应利用节拍的安排来减少跳进所招致的注意：在可能时，从强拍跳到弱拍（在小节之内），而不是从弱拍跳到强拍（跨过小节线）。
...
此外，由于第二类对位的线条中音符更多，通常应有一到两个*次级高潮*——低于整体高潮的音，作为线条某些局部的“小高潮”。这有助于线条的完整性，确保它具备连贯的形态，而不是漫无目的地游走。

### Beginning a second-species counterpoint｜开始写作第二类对位
...
与第一类对位一样，在固定旋律之上写作第二类对位时，以 *do* 或 *sol* 开始。在固定旋律之下写作第二类对位时，以 *do* 开始。练习的第一个和最后一个二音组合允许使用同度。
...
第二类对位的线条可以第一小节用两个二分音符开始，也可以用二分休止符后接一个二分音符开始。以二分休止符开始能更快确立节奏轮廓，使听者更易解析，因此往往更可取。写作起来也更容易。无论节奏如何，对位声部的第一个音高都应遵循上述音程规则。

### Ending a second-species counterpoint｜结束第二类对位

The final pitch of the counterpoint should be *do*, as in first species.

与第一类对位一样，对位声部的最后一个音高应为 *do*。

The penultimate note of the counterpoint should be *ti* if the cantus is *re*, and *re* if the cantus is *ti*, as in first species.

与第一类对位一样，如果固定旋律是 *re*，对位声部的倒数第二个音应为 *ti*；如果固定旋律是 *ti*，则应为 *re*。
...
对位声部的倒数第二小节可以是*一个全音符*（这样最后两小节就与第一类对位完全相同），也可以是两个二分音符。采用哪一种取决于你如何接近最后一小节。（这只是一项历史惯例，并非音乐上的必然。但这一额外的自由度使进入最终到达时更容易保持平滑，而不必增添太多复杂因素。）
...
*由于在音乐织体中引入不协和会产生需要处理的新音乐问题，第二类对位以非常有限的方式引入不协和。这并非音乐上的必然，也不是处理不协和的唯一方式，但它通过在每个种类中只引入少量新的音乐难点而有所帮助。*
...
第二类对位中的强拍（小节的第一拍）*永远是协和的*。与第一类对位一样，优先使用不完全协和音程（三度与六度）而非纯协和音程（五度与八度），并避免同度。
...
由于跨小节线的进行（从弱拍到强拍）涉及与第一类对位相同类型的声部进行（两声部同时移动），应遵循与第一类对位相同的原则。例如，如果某个弱拍是纯五度，紧随其后的小节第一拍就不能也是纯五度。
...
同样，从小节第一拍进行到小节第一拍也必须遵循第一类对位的原则。以下是一些示例，但并非穷尽性的清单：
...
- Do not begin two consecutive bars with the same perfect interval.／不要以相同的纯音程开始连续的两个小节。
- Do not outline a dissonant melodic interval between consecutive downbeats. (Exception: if the counterpoint leaps an octave from the strong beat to the weak beat, the leap should be followed by step in the opposite direction making a seventh with the preceding downbeat. This is okay, since it is the result of smooth voice motion.)／不要在相邻的小节第一拍之间勾勒出不协和的旋律音程。（例外：如果对位声部从强拍到弱拍跳进一个八度，该跳进之后应朝相反方向级进，从而与前面的小节第一拍构成一个七度。这是可以的，因为它是平滑声部进行的结果。）
- Do not begin more than three bars in a row with the same imperfect consonance.／不要以相同的不完全协和音程开始连续超过三个小节。
...
相邻小节第一拍之间的*隐伏*或*直接五度／八度*是可以的，因为其效果很弱，而且对位声部中间插入的那个音会削弱这一效果。
...
由于和声不协和音程可以出现在弱拍上，弱拍上协和与不协和音程的混合是促进变化的最好方式。
...
同度在第一类对位中是有问题的，因为它削弱了线条的独立性。然而，当它们出现在第二类对位的弱拍上，并且是其他方面平滑的声部进行所导致的结果时，线条在节奏上的差异已足以维持那种独立性。因此，在为使线条之间形成良好对位而有必要时，弱拍上允许使用同度。
...
任何弱拍上的不协和都必须遵循下文所述的*不协和经过音*模式。下文同样会解释若干弱拍协和音的标准模式。如果你的弱拍不符合下面任何一种模式，那很可能意味着对位存在问题，因此请把这些模式既作为写作对位的指南，也作为评估对位的手册。

### Weak beat patterns｜弱拍模式
...
以下模式（其术语或为通行用法，或取自 Salzer 与 Schachter 的《Counterpoint in Composition》）应作为你在第二类对位线条中使用弱拍音的指南。一个良好的常规做法是：先写一个小节第一拍的音，再选定下一个小节第一拍的音，最后从下面的模式中选择一个，使你能够很好地填充两个小节第一拍之间的空间。

Most of these are used as examples in the demonstration video at the bottom of the page.

其中大多数都在页面底部的演示视频中作为示例使用。

#### Dissonant weak beats｜不协和弱拍
...
*第二类对位中所有不协和的弱拍都是不协和经过音*，之所以这样称呼，是因为对位线条以级进从一个协和的小节第一拍经过到另一个协和的小节第一拍。对位声部中从小节第一拍到小节第一拍的旋律音程永远是三度，经过音则位于中间，以经过运动填充这个三度。
...
由于第二类对位中所有的不协和都是经过音，你绝不会跳进或跳出某个不协和的音，也不会在不协和的音上改变方向，更不会有任何不协和出现在小节第一拍上。

<iframe src="https://trinket.io/embed/music/ef1cff1ee3" width="100%" height="300" frameborder="0" allowfullscreen></iframe>

#### Consonant weak beats｜协和弱拍
...
*协和经过音*从小节第一拍到小节第一拍勾勒出一个三度，其模式与不协和经过音相同，只是三个音（小节第一拍、经过音、小节第一拍）都与固定旋律协和。协和经过音永远是位于固定旋律上方／下方的六度或纯五度。

<iframe src="https://trinket.io/embed/music/de54f539bb" width="100%" height="300" frameborder="0" allowfullscreen></iframe>
...
*替代音型*同样从小节第一拍到小节第一拍勾勒出一个三度。然而，它不是用级进填充，而是由对位声部跳进一个四度，再朝相反方向级进。称之为替代，是因为在需要额外的跳进或方向改变以提供变化的线条中，它可以替代经过音。与协和经过音一样，对位声部中的三个音都必须与固定旋律协和。

<iframe src="https://trinket.io/embed/music/f258be56b6" width="100%" height="300" frameborder="0" allowfullscreen></iframe>
...
*跳过经过音*从小节第一拍到小节第一拍勾勒出一个四度。弱拍上的音把这个四度分割为一个三度和一个级进。同样，三个音程（小节第一拍、跳过经过音、小节第一拍）都与固定旋律协和。

<iframe src="https://trinket.io/embed/music/24c7f0ee8e" width="100%" height="300" frameborder="0" allowfullscreen></iframe>
...
*音程细分*在相邻的小节第一拍之间勾勒出一个五度或六度。小节第一拍之间那个协和的大旋律音程，被分割为两个较小的协和跳进。小节第一拍之间的旋律五度会被分为两个三度。小节第一拍之间的旋律六度会被分为一个三度和一个四度，或一个四度和一个三度。不仅三个*旋律*音程都必须协和（包括各相邻音之间的音程以及小节第一拍之间的音程），而且对位声部中的每个音都必须与固定旋律协和。

<iframe src="https://trinket.io/embed/music/a79b68de03" width="100%" height="300" frameborder="0" allowfullscreen></iframe>
...
*音区转换*出现在这样一种情形：从强拍到弱拍的一个协和大跳（P5、六度或八度）之后，朝相反方向级进。它用于在长段级进之后获得旋律上的变化，用于避免平行进行或其他问题，或用于让开固定旋律以保持独立性。它应不常使用。一如既往，每个音都必须与固定旋律协和。

<iframe src="https://trinket.io/embed/music/827ad0b4a2" width="100%" height="300" frameborder="0" allowfullscreen></iframe>
...
*旋律进行延迟*从小节第一拍到小节第一拍勾勒出一个级进。它包含从强拍到弱拍的一个三度跳进，随后朝相反方向级进进入下一个小节第一拍。称之为“延迟”，是因为它用来装饰原本更为缓慢的第一类对位进行（从小节第一拍到小节第一拍的级进运动）。

<iframe src="https://trinket.io/embed/music/5764fe4a4b" width="100%" height="300" frameborder="0" allowfullscreen></iframe>
...
*协和辅助音*出现在这样一种情形：对位声部从小节第一拍级进到弱拍，随后在下一个小节第一拍回到原来的音高。如果第一个小节第一拍与固定旋律构成五度，协和辅助音就会构成六度，反之亦然。

<iframe src="https://trinket.io/embed/music/043d342ffa" width="100%" height="300" frameborder="0" allowfullscreen></iframe>
...
在下面的视频中，我演示写作第二类对位的过程。该视频提供了关于写作过程的新信息，以及上述规则与原则的具体示例。

<iframe src="http://player.vimeo.com/video/57085974" width="100%" height="400" frameborder="0" allow="autoplay; fullscreen; picture-in-picture" allowfullscreen></iframe>
