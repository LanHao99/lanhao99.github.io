...
description: "四对一音符的严格二声部写作规则与检查清单。"
...
sourceLink: https://openmusictheory.github.io/thirdSpecies.html
comment: true
draft: false
---
...
在第三类对位中，对位线条以四分音符运动，与全音符写成的固定旋律相对。这种 4:1 的节奏比例造成了比第二类对位更大的拍子区分度：*强拍*（小节的第一拍）、*次强拍*（每小节的第三个四分音符）与*弱拍*（每小节的第二与第四个四分音符）。第三类对位还引入了*辅助音*不协和，以及两个与之相关的、其中不协和可以参与跳进的音型。
...
与第一、第二类对位一样，对位线条应便于歌唱、形态良好、只有一个不与固定旋律高潮重合的高潮，并以级进为主（为求变化可有一些小跳和偶尔的大跳）。与第二类对位一样，第三类对位比第一类*更加以级进为主导*，因为需要跳进来应付的棘手情形更少。如果对位必须跳进，宁可选择在小节之内跳进，而不是跨过小节线。同样与第二类对位一样，通常应有一到两个*次级高潮*——低于整体高潮的音，作为线条某些局部的“小高潮”。

### Beginning a third-species counterpoint｜开始写作第三类对位
...
在固定旋律之上写作第三类对位时，以 *do* 或 *sol* 开始。在固定旋律之下写作第三类对位时，以 *do* 开始。练习的第一个和最后一个二音组合允许使用同度。
...
第三类对位的线条可以第一小节用四个四分音符开始，也可以用四分休止符后接三个四分音符开始。无论节奏如何，对位声部的第一个音高都应遵循上述音程规则。

### Ending a third-species counterpoint｜结束第三类对位

The final pitch of the counterpoint must always be *do*, and must be a whole note.

对位声部的最后一个音高必须始终是 *do*，且必须是全音符。
...
对位声部的倒数第二个音（倒数第二小节的最后一个四分音符），如果固定旋律是 *re* 则应为 *ti*，如果固定旋律是 *ti* 则应为 *re*。

### Strong beats｜强拍

Principles for strong beats (downbeats) are generally the same as in second species.

强拍（小节第一拍）的原则与第二类对位大体相同。
...
从小节第一拍到小节第一拍的进行遵循第二类对位的原则，但有一处例外（见下文）。以下是一些示例，但并非穷尽性的清单：
...
- No *three* consecutive bars can begin with the same perfect interval (two in a row are fine).／不得有连续*三个*小节以相同的纯音程开始（连续两个是可以的）。
- No more than three bars in a row should begin with the same imperfect consonance.／以相同的不完全协和音程开始的小节不应连续超过三个。
- The pitches that begin consecutive downbeats must not make a dissonant melodic interval.／开始相邻各个小节第一拍的音高之间，不得构成不协和的旋律音程。
...
如果某个小节第一拍包含纯五度，那么前一小节的第三拍或第四拍都不能是五度。如果某个小节第一拍包含八度，那么前一小节的第二、第三或第四拍都不能是八度。与第二类对位一样，平行五度与平行八度的负面效果并不会因为多加了一两个音而被抵消。

*Hidden* or *direct fifths/octaves* between successive downbeats are allowed.

相邻小节第一拍之间的*隐伏*或*直接五度／八度*是允许的。
...
第 2–4 拍应呈现协和与不协和音程的混合，以促进变化。在协和音程中，当为使线条之间形成良好对位而有必要时，弱拍上允许使用同度。任何不协和都必须遵循下文所述的*不协和经过音*或*不协和辅助音*模式。下文同样会解释若干弱拍协和音的标准模式。

#### Harmonic dissonances｜和声不协和
...
一般而言，第三类对位中的不协和可以出现在第 2、3 或 4 拍上，并且应*之前与之后都是级进*（下文所述*双重辅助音*与*换音*（nota cambiata）除外）。这能促进平滑，既通过让不协和避开小节中最强的拍子，也通过把它们与最平滑的旋律运动结合。如果对位声部中所有不协和的音都遵循下列模式之一，它们应会产生悦耳的效果。否则，它们可能听起来刺耳、悬而未决，或者难以歌唱。
...
*不协和经过音*以级进填充一个旋律三度的空间。经过音之前和之后的音都必须与固定旋律协和。
...
请注意，可以连续使用两个不协和经过音（**P4–d5** 或 **d5–P4**）。只要这些不协和不落在小节第一拍上，且对位声部朝单一方向级进运动，就没有负面效果。
...
*不协和辅助音*通过离开一个协和的音再级进回到原来的协和音来装饰它（例如在固定旋律上方构成 6–7–6）。它在旋律上与第二类对位的协和辅助音完全相同，区别只在于和声上的不协和。把它用在弱拍（第 2 或第 4 拍）上可确保最大的平滑度。
...
*双重辅助音*出现在这样一种情形：对位声部中的第 1 拍与第 4 拍是同一个音，而第 2 与第 3 拍包含比原音高一级和低一级的音。例如 C–D–B–C 或 C–B–D–C。第 2 与第 3 拍都是不协和的，但由于两者都是以级进装饰原音，它们之间的跳进并不会显著削弱线条的平滑。使用双重辅助音时，第 3 拍与第 4 拍之间的方向应与第 4 拍与随后小节第一拍之间的方向相同。跨过小节线的那一进行同样应为级进。这进一步保持了平滑，以缓和这些不协和的效果。
...
*nota cambiata*（换音）是一个五音音型，从小节第一拍到小节第一拍勾勒出一个级进进行。它遵循以下两种模式之一：

- down by step – down by third – up by step – up by step
- up by step – up by third – down by step – down by step

- down by step – down by third – up by step – up by step／下行级进 – 下行三度 – 上行级进 – 上行级进
- up by step – up by third – down by step – down by step／上行级进 – 上行三度 – 下行级进 – 下行级进
...
第一种模式会形成从小节第一拍到小节第一拍的下行级进，第二种模式则形成上行级进。要使 *nota cambiata* 有效，第一个、第三个与第五个音*必须与固定旋律协和*。第二个音是不协和的，并将跳进到第三个音。然而，与双重辅助音一样，这一整体模式把离开不协和的跳进所带来的负面效果降到最低：它被级进所围绕，整体进行只有一个级进，而且不协和的音与随后的小节第一拍是同一音高。
...
对位声部可以自由地以级进进入和离开协和的音，也可以从另一个协和音跳进进入，但有以下几点需要考虑：
...
- All melodic leaps, of course, must be melodic consonances.／所有旋律跳进当然都必须是旋律协和音程。
- A large leap should be followed by a step in the opposite direction.／大跳之后应朝相反方向级进。
- Motion from the fourth beat into the following downbeat should follow the constraints above for motion into strong beats.／从第四拍进入随后小节第一拍的进行，应遵循上文关于进入强拍的限制。
