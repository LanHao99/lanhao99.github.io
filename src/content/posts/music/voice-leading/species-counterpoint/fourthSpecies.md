...
sourceLink: https://openmusictheory.github.io/fourthSpecies.html
comment: true
draft: false
---
...
在第四类对位中，对位线条与固定旋律都每小节运动一次，但两者在节奏上彼此错开一个二分音符。（可以把它想成小节层面上的切分。）对位线条将以二分音符记谱，其中每个弱拍上的二分音符都跨过小节线延音连结到随后的强拍。这样的安排意味着，在纯粹的第四类对位中，两条线条总是以斜向进行移动。它还引入了一种新的不协和：*延留音*。
...
*延留音*是一种被强调的不协和，也就是说它总是出现在强拍上。由于所受到的强调更强，必须格外留意以促进平滑与整体的连贯。因此，与*经过音*、*辅助音*这两种不协和一样，延留音之前与之后永远是协和的和声音程。
...
- the *preparation*: a weak-beat note in the counterpoint that is consonant with the cantus. This note will be tied into . . .／*准备*：对位声部中与固定旋律协和的一个弱拍音。这个音将被延音连结进入……
- the *suspension* itself: a strong-beat note in the counterpoint that is dissonant with the cantus. This note is the same as the preparation.／*延留*本身：对位声部中与固定旋律不协和的一个强拍音。这个音与准备音相同。
- the *resolution*: a weak-beat note in the counterpoint that is consonant with the cantus. It will always be a step lower than the suspended tone.／*解决*：对位声部中与固定旋律协和的一个弱拍音。它永远比延留的音低一个级进。
...
在第四类对位中应尽可能多地使用不协和延留音。它们不仅是第四类对位的标志性音响，而且听起来悦耳，并且在第四类对位中正确运用它们，会为你日后在创作与编配工作中运用延留音和不协和和弦音做好准备。

### Types of suspensions｜延留音的类型
...
延留音按照*延留*音与*解决*音位于固定旋律上方／下方所构成的音程来分类。例如，**7–6 延留音**包含一个与固定旋律构成七度的强拍延留音，它下行级进解决到一个与固定旋律构成六度的弱拍音。
...
固定旋律*上方*可能的不协和延留音为 **7–6**、**4–3** 与 **9–8**。（这些是唯一的几个从某个不协和开始、下行级进解决到一个允许使用的协和音程的选项。）固定旋律*下方*可能的不协和延留音为 **2–3**、**5–6** 与 **4–5**。（7–8 在理论上是可能的，但它的音响往往不如其他几个悦耳。最好避免。）

### Using suspensions｜延留音的运用
...
在第四类对位中处理延留音时，应像在第一类对位中处理它们的解决音程那样处理。例如，不要连续使用两个 **9–8** 或 **4–5** 延留音（因为在第一类对位中不能连续使用两个八度或两个五度）。可自由使用 **7–6** 与 **4–3**（上方）或 **2–3** 与 **5–6**（下方），但连续不超过三次（如同第一类对位中的三度与六度）。
...
依照同样的原则，“协和延留音” **6–5** 也不得连续使用两次，因为它的“解决”音程是一个五度。事实上，由于第四类对位线条所确立的模式会引导听者把弱拍理解为主要协和音，在第四类对位中应避免任何会在相邻弱拍上产生两个五度或两个八度的配置（称为“后拍”五度或八度）。

## The fourth-species counterpoint line｜第四类对位的对位线条
...
只要可能就使用不协和延留音。这会形成一条主要由下行级进构成的线条。这没有问题。这也会使把运动指向某个高潮变得困难。这同样没有问题。只要线条平滑、便于歌唱，且延留音得到妥善的准备与解决，就不必为它的形态操心。（要写出与固定旋律形态相同的第四类对位实在太难，而且第四类对位在教学上的要点就在于延留音的处理。因此我们暂时忽略旋律形态，以磨炼第四类对位中的延留音技巧。）
...
如果无法使用不协和延留音，就试着从弱拍到强拍使用延音连结。这可以是一个“协和延留音”，或者你也可以从小节第一拍的协和音向上跳进到弱拍上的协和音。至少需要一到两次向上跳进，以抵消那些下行解决，从而使线条保持在便于歌唱的音域之内。
...
如果既无法使用不协和延留音，也无法使用协和的延音连结音型，则允许*打破种类*（见下方视频演示）。当你打破种类时，应遵循第二类对位的原则，并尽快恢复第四类对位的延音连结。尽量不要在一条练习中打破种类超过一次，且只持续一两个小节。

### Beginning a fourth-species counterpoint｜开始写作第四类对位
...
在固定旋律之上写作第四类对位时，以 *do* 或 *sol* 开始。在固定旋律之下写作第二类对位时，以 *do* 开始。练习的第一个和最后一个二音组合允许使用同度。

Always begin with a half rest.

永远以二分休止符开始。

### Ending a fourth-species counterpoint｜结束第四类对位
...
固定旋律*必须*以 *re*–*do* 结束。不要使用以 *ti*–*do* 结束的固定旋律。
...
对位声部将以一个不协和延留音结束。倒数第二小节将包含 *do*–*ti*，最后一小节将包含一个全音符 *do*。这个 *do*–*ti* 会在固定旋律的 *re* 之上构成 **7–6** 延留音，或在固定旋律的 *re* 之下构成 **2–3** 延留音。作为一个不协和延留音，那个 *do* 永远是从前一小节延音连结过来的。
...
在下面的视频中，我演示在固定旋律之上和之下写作第四类对位的过程。该视频提供了关于写作过程的新信息，以及上述规则与原则的具体示例。

<iframe src="http://player.vimeo.com/video/57389373" width="100%" height="400" frameborder="0" allow="autoplay; fullscreen; picture-in-picture" allowfullscreen></iframe>

[Composing a fourth-species counterpoint above a cantus firmus](http://vimeo.com/57389373) from [Kris Shaffer](http://vimeo.com/user11692346) on [Vimeo](http://vimeo.com).

[在固定旋律之上写作第四类对位](http://vimeo.com/57389373)，来自 [Kris Shaffer](http://vimeo.com/user11692346)，发布于 [Vimeo](http://vimeo.com)。

<iframe src="http://player.vimeo.com/video/58488043" width="100%" height="400" frameborder="0" allow="autoplay; fullscreen; picture-in-picture" allowfullscreen></iframe>

[Composing a fourth-species counterpoint below the cantus firmus](http://vimeo.com/58488043) from [Kris Shaffer](http://vimeo.com/user11692346) on [Vimeo](http://vimeo.com).

[在固定旋律之下写作第四类对位](http://vimeo.com/58488043)，来自 [Kris Shaffer](http://vimeo.com/user11692346)，发布于 [Vimeo](http://vimeo.com)。
...
下面是一条第四类对位的范例作品。其中有一处“规则”被“打破”的例子。找出它，并尝试重写这条练习。你能给出一个比原作更具音乐说服力的“正确”解法吗？如果不能，你认为原因何在？

<iframe src="https://trinket.io/embed/music/22aa934458" width="100%" height="300" frameborder="0" allowfullscreen></iframe>
