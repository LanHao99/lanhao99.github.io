...
description: "一对一音符的严格二声部写作规则与检查清单。"
...
sourceLink: https://openmusictheory.github.io/firstSpecies.html
comment: true
draft: false
---
...
对位是把两条或更多音乐*线条*调解为一个有意义而悦耳的*整体*。在第一类对位中，我们不仅要写出一条自身在形态上完整、具有变化与目标导向运动的平滑旋律，还要写出同样具备这些特性的第二条旋律。更进一步，也是最重要的，我们要把这些旋律组合起来，创造出一个平滑、具有变化与目标导向运动、并且其中各条旋律既保持独立又融合为协和*同时发响音*（两个或更多音同时发响的统称）的整体织体。
...
在第一类对位中，我们从一条*固定旋律*（新写的或既有的）开始，在固定旋律之上或之下写作一条新的线条，称为*对位声部*。这条新线条对固定旋律中的每个音都对应一个音：固定旋律与对位声部都将全部是全音符。因此，第一类对位有时也被称为一对一或 1:1 对位。

## The counterpoint line｜对位线条

In general, the counterpoint should follow the principles of [writing a good cantus firmus](/posts/music/voice-leading/species-counterpoint/cantusFirmus/). There are some minor differences, to be discussed below, but generally a first-species counterpoint should consist of two cantus-firmus-quality lines.

一般而言，对位声部应遵循[写作良好固定旋律](/posts/music/voice-leading/species-counterpoint/cantusFirmus/)的原则。其中有一些细微差别，将在下文讨论，但总的来说，一个第一类对位应由两条具备固定旋律品质的线条构成。

### Beginning a first-species counterpoint｜开始写作第一类对位
...
为体现目标导向的运动，第一类对位练习应以最稳定的音响——纯协和音程——开始并结束。因此，在固定旋律*之上*写作对位时，对位声部的第一个音应是 *do* 或 *sol*（与固定旋律构成 P1、P5 或 P8）。
...
在固定旋律*之下*写作对位时，对位声部的第一个音必须始终是 *do*（与固定旋律构成其下方的 P1 或 P8）。（以 *sol* 开始会产生不协和的四度；以 *fa* 开始会产生 P5，但会让听者对调性语境感到困惑，因为作品开头的 *fa–do* 很容易被误听为 *do–sol*。）

### Ending a first-species counterpoint｜结束第一类对位

The final note of the counterpoint must always be *do* (P1 or P8 above/below the cantus).

对位声部的最后一个音必须始终是 *do*（与固定旋律构成其上／下方的 P1 或 P8）。
...
为平滑、富于变化且具有强烈目标导向地进入这一结束，应始终以反向级进接近最终的音程。如果固定旋律以 *re*–*do* 结束，对位声部最后两个音高应为 *ti*–*do*。如果固定旋律以 *ti*–*do* 结束，对位声部最后两个音高应为 *re*–*do*。因此倒数第二小节中两条线条之间要么是小三度，要么是大六度。大调与小调均如此。

### Independence of the lines｜线条的独立性
...
与固定旋律一样，对位声部应只有一个高潮。为保持各线条的独立性以及整段的平滑流畅（以免某一瞬间因双重高潮而被过度强调），这两个高潮不应重合。
...
对位声部中允许有一次重复／延音连结，但尽量完全避免重复。这会增进练习中的变化，因为原本音符就很少。
...
避免*声部交错*，即上方声部暂时低于下方声部，反之亦然。声部交错会削弱线条的独立性，并使它们更难靠听觉分辨。
...
避免*声部重叠*，即某一声部跳进超过另一声部前一个音的位置。例如，若上方声部唱 E4，下方声部就不能在下一小节唱 F4。这同样有助于保持线条的独立性。

### Intervals and motion｜音程与进行方式
...
任何时刻固定旋律与对位声部之间的音程都不应超过纯十二度（八度加五度）。一般而言，尽可能把两条线条保持在一个八度以内，只有在“紧急情况”下才超过十度，且仅为暂时（一两个音）。当两声部相距过远时，调性融合会被削弱。此外，它还会削弱*可演奏（唱）性*——尽管这不是人类认知的本质原理，却是作曲家必须考虑的重要问题，并且它直接影响听者在演奏中所听到的平滑流畅、旋律完整性与调性融合。
...
一般而言，所有和声*协和音程*都是允许的。然而，同度只应用于第一个和最后一个音程。同度极为稳定，最适合作为目标而非中途的经停点。它们还会削弱线条的独立性。
...
除第一个和最后一个二音组合之外，所有音程都更宜使用*不完全协和音程*而非*纯协和音程*，以加强结束处的到达感，并促进朝这一到达前进的运动感。无论如何，都要力求在练习进程中使和声音程多样化。
...
*绝对、绝对、绝对*不要连续使用两个大小相同的纯协和音程：**P5–P5** 或 **P8–P8**。这既包括单音程也包括复音程。例如，**P5–P12** 与 **P5–P5** 被视作相同。（不过，连续使用两个不同的纯协和音程，如 **P8–P5**，是允许的，但如有可能，应尽量在每个纯协和音程之后接一个不完全协和音程。）这些“平行五度与平行八度”在显著提升调性融合的同时削弱了旋律独立性，而连续的稳定音响又同时抑制了练习的变化与运动。因此，它们远非理想，在种类对位中应当避免。
...
要变化相邻音程之间的进行方式类型（平行、同向、反向、斜向）。尽量使用所有类型的进行方式（或许斜向除外），但在可能时优先使用反向进行。它在保持线条独立性方面最为有效，同时也有助于变化。
...
由于同向与平行进行会削弱变化与旋律独立性，使用它们时应由其他因素加以调节：
...
- Do not use more than three of the same imperfect consonance type in a row (e.g., three thirds in a row).／同一类不完全协和音程连续使用不超过三次（例如连续三个三度）。
- *Never* move into a perfect consonance by similar motion (this is called *direct* or *hidden octaves*). This draws too much attention to an interval which already stands out of the texture.／*绝不*以同向进行进入纯协和音程（这被称为*直接*或*隐伏八度*）。那会把过多注意力引向一个本就突出于织体之外的音程。
- Avoid combining similar motion with leaps, especially large ones.／避免把同向进行与跳进结合使用，尤其是大跳。
...
在下面的视频中，我演示写作第一类对位的过程。该视频提供了关于写作过程的新信息，以及上述规则与原则的具体示例。

<iframe src="http://player.vimeo.com/video/56828309" width="100%" height="400" frameborder="0" allow="autoplay; fullscreen; picture-in-picture" allowfullscreen></iframe>
...
在从零开始写作第一类对位练习之前，先试试下面这些练习。每一条都有一到两处错误。请试着找出错误，并重写这条练习，使之成为结构规整的练习。（注意：中音谱号那条是*固定旋律*。只改动对位线条。）务必聆听并演奏这些练习，既按它们原本的写法，也按你改动后的样子。你的耳朵或许已经能指引你找到错误。如果不能，就用上文逐条列出的原则来查找错误。一旦找出某处错误，务必反复聆听数次，一边跟着其中一条线条歌唱，以训练你的耳朵识别这一问题。

### Minor key practice, counterpoint above｜小调练习，对位在上方

<iframe src="https://trinket.io/embed/music/a089e987ca" width="100%" height="300" frameborder="0" allowfullscreen></iframe>

### Major key practice, counterpoint below｜大调练习，对位在下方

<iframe src="https://trinket.io/embed/music/585810f762" width="100%" height="300" frameborder="0" allowfullscreen></iframe>
