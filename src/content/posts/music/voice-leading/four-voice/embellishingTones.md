...
sourceLink: https://openmusictheory.github.io/embellishingTones.html
comment: true
draft: false
---
...
![经过音](/media/music/graphics/embellishingTones/passingTone.png)
...
经过音是一种旋律装饰（通常为和弦外音），出现在两个稳定音（通常为和弦音）之间，从而形成级进运动。典型形态是*和弦音 – 经过音 – 和弦音*，填充一个三度（见谱例），但两个相邻的经过音也可用来填充相隔四度的两个和弦音之间的空间。经过音既可以是强位（出现在强拍或拍中的强位）的，也可以是弱位（弱拍或拍中的弱位）的。

## Complete Neighbor Tone (NT)｜完全辅助音（NT）

![辅助音](/media/music/graphics/embellishingTones/neighborTone.png)
...
与经过音一样，完全辅助音是出现在两个稳定音（通常为和弦音）之间的旋律装饰；然而，完全辅助音出现在同一个稳定音的两个实例之间。同样与经过音一样，从稳定音到辅助音再返回的进行总是级进。完全辅助音既可以是强位的，也可以是弱位的，但弱位更为常见。

## Double Neighbor Figure (DN)｜双辅助音型（DN）

![双辅助音](/media/music/graphics/embellishingTones/doubleNeighbor.png)
...
与完全辅助音型一样，双辅助音型起止于*同一个*稳定音（通常为和弦音）。在该稳定音的两个实例之间有两个装饰音——一个位于被装饰的稳定音上方一度，另一个位于其下方一度。虽然单独来看，我们可能把这两个装饰音各自视为不完全辅助音（见下文），但它们在双辅助音型中协同作用、彼此平衡，构成一个连贯的整体，具有与完全辅助音相同的整体稳定性。双辅助音型通常是弱位的。

## Incomplete Neighbor Tone (INT)｜不完全辅助音（INT）

![不完全辅助音](/media/music/graphics/embellishingTones/INT.png)
...
不完全辅助音是一种弱位的装饰音，以跳进进入，再级进进行到一个强位的稳定音（通常为和弦音）。广义而言，不完全辅助音是任何距稳定音一度之遥、并接在其前或后（而另一侧以跳进相连）的装饰音，但其他类型的不完全辅助音有专门的名称与作用，见下文。

## Appoggiatura (APP)｜倚音（APP）

![倚音](/media/music/graphics/embellishingTones/appoggiatura.png)
...
倚音是一种不完全辅助音，为强位，以跳进进入（通常是向上跳进），随后级进（通常向下，但总是与之前的跳进方向相反）进行到一个更为稳定的音（通常为和弦音）。

## Escape Tone (ESC)｜逃离音（ESC）

![逃离音](/media/music/graphics/embellishingTones/escapeTone.png)
...
逃离音，或称 *echappée*，是一种不完全辅助音，为弱位，由和弦音级进（通常向上）进入其前，随后跳进（通常向下，但总是与之前的级进方向相反）。

## Anticipation (ANT)｜先现音（ANT）

![先现音](/media/music/graphics/embellishingTones/anticipation.png)
...
先现音本质上是一个本应稳定、却来得太早的音。先现音通常是在和声变化之前立即出现的和弦外音，并在该和声变化处由同一个音紧随其后，此时该音已成为新和声的和弦音。它通常出现在乐句以及更大结构单位的结尾。

## Syncopation (SYN)｜切分（SYN）

![切分音](/media/music/graphics/embellishingTones/syncopatedNote.png)
...
[Syncopation｜切分](http://openmusictheory.com/syncopation.html)出现在一个通常发生于强拍或拍中强位的节奏型，转而发生于弱拍或拍中弱位之时。与先现音一样，切分音是提前到达的——它倾向于属于下一拍的和弦。与先现音不同的是，切分音与那个和弦中的某个音相连接；它并不被重新发声。切分与其说是在预示后续和弦中的某个音，不如说只是提前到达而已。

## Suspension (SUS)｜延留音（SUS）

![延留音](/media/music/graphics/embellishingTones/suspension.png)
...
延留音由三个关键部分构成：*准备*（强位或弱位）、*延留*本身（强位）以及*解决*（弱位）。准备是一个和弦音（协和音）。延留与准备是*同一个音*，并与和声变化同时发生。随后延留音向下级进至解决，解决与延留处在同一和声之上。延留音在许多方面与切分相反：若先现音是属于后续和弦的某个音提前到达，那么延留音就是属于前一和弦的某个和弦音迟迟不离，从而迫使新和弦的和弦音迟迟到不了位。然而，在作曲与即兴中，延留音的处理必须比切分谨慎得多。上方声部中最常见的延留音（及其解决）相对低音形成如下音程样式：9–8、7–6、4–3。（除 9–8 之外，解决音的音级绝不应在与延留音同时的另一个声部中发声。）相较于标写 *SUS*，更常见的做法是在通奏低音记号中记出该音程样式。

## Retardation (RET)｜上行延留音（RET）

![上行延留音](/media/music/graphics/embellishingTones/retardation.png)
...
上行延留音实质上是一种向上解决的延留音。它几乎总是专用于大型结构划分（或乐章）的最终和弦，且常与延留音同时出现（如谱例所示）。相较于标写 *RET*，更好的做法是在通奏低音记号中记出该音程样式。
