...
description: "倾向音的解决逻辑，以及功能性和声语境中不协和音的处理。"
...
sourceLink: https://openmusictheory.github.io/tendencyTonesFunctionalDissonances.html
comment: true
draft: false
---
...
在严格键盘织体中，有两类主要音高倾向需要谨记：*倾向音*与*功能性不协和音*。
...
*倾向音*是某一音（级）——通常以音级来表示——它倾向于进行到某些音级胜过其他音级。有时这种倾向在某风格中是绝对的，但更常见的是取决于语境。

The most prominent tendency tones in Western tonal styles are *ti* (not *te*) and *le* (not *la*).

西方调性风格中最突出的倾向音是 *ti*（而非 *te*）与 *le*（而非 *la*）。
...
一般来说，当 *ti* 出现时，它倾向于在同一音部中由 *do* 紧随其后。在和声语境中，当 *ti* 出现在具有属功能的和弦中时，这一倾向最为强烈，而该倾向的“解决”发生在功能变化（转向主功能或下属功能）之时。

Likewise, when *le* appears, it tends to be followed by *sol* in the same voice. This tendency is less dependent on function.

同样，当 *le* 出现时，它倾向于在同一音部中由 *sol* 紧随其后。这一倾向对功能的依赖较小。

Exceptions to these tendencies include:

这些倾向的例外情况包括：

- When *ti* is in the middle of a stepwise descent (*re*–*do*–*ti*–*la*–*sol*, for example), it can progress down by step. (Note that *step inertia* here diminishes the effect of an “unresolved” tendency tone. Because there are two conflicting tendencies in play, in this case, either can be “resolved” unproblematically.)／当 *ti* 位于级进下行的中间时（例如 *re*–*do*–*ti*–*la*–*sol*），它可以向下级进。（请注意，此处的*级进惯性*削弱了“未解决”倾向音的效果。由于此时有两种相互冲突的倾向在起作用，任一种都能毫无问题地被“解决”。）
- When *ti* is in an inner voice, it can progress down to *sol* if necessary to accomplish good voice-leading in the other voices and ensure complete chords. This is called a *frustrated leading-tone*.／当 *ti* 位于内声部时，若为在其他声部中实现良好的声部进行并确保和弦完整而有必要，它可以下行至 *sol*。这称为*受挫的导音*。
- When *ti* is a functional dissonance of a tonic-functioning chord (see below) it should progress down by step.／当 *ti* 是主功能和弦的功能性不协和音时（见下文），它应向下级进。

## Functional dissonances｜功能性不协和音
...
有些倾向，例如 *le* 向下进行的倾向，相对不依赖语境。另一些则高度依赖语境。关于旋律音如何进行的首要语境性倾向，就是*功能性不协和音*这一概念。

Keep in mind from the [Harmonic functions resource](/posts/music/harmony/harmonicFunctions/) that chords tend to cluster in one of three functional groups (**T**, **S**, or **D**) When pitches fuse into a chord expressing one of these three functions, the pitches that comprise that have certain tendencies of progression that they may or may not have in other contexts.

请记住[和声功能资源](/posts/music/harmony/harmonicFunctions/)中的内容：和弦倾向于聚集在三个功能组（**T**、**S** 或 **D**）之一中。当各音高融合为一个表达这三种功能之一的和弦时，构成该和弦的各音就具有某些进行倾向，而这些倾向在其他语境中未必存在。

Following are the scale degrees which act as dissonances for their respective functions:

以下是在各自功能中充当不协和音的音级：

| function: 功能 | dissonances: 不协和音 |
| ---: | :---: |
| T or Tx／T 或 Tx | 7, 5 when 6 is also present／当 6 同时出现时的 7、5 |
| S／S | 3, 1 when 2 is also present／当 2 同时出现时的 3、1 |
| D／D | 4, 6 |
...
在纯粹自然声音乐中（三和弦与七和弦，无半音变化），这些将包括*每个七和弦的七音*、*viiº 或 VII 的五音*（*fa*），以及 *III 或 iii 的五音*（*ti/te*）。
...
请记住，这些功能性不协和音只在某些时候表现为音响上不协和的和弦或音程。然而，它们确实引入了一定程度的张力，而这种张力如同分类对位中的音响不协和音程一样，需要平滑的引入与特定的解决。
...
当这些音级之一出现在具有相应功能的和弦中时，该不协和音级有强烈的倾向，要*在下一次功能变化时向下级进解决*。在严格写作中，我们将*始终*遵循这些倾向。
...
在严格键盘织体中，这些功能性不协和音应通过共同音或级进来“准备”（进入）。因此，虽然它们是和弦的正式成员，但在旋律上它们会呈现出分类对位三种不协和音型之一的样貌：以级进进入的*经过音*或*辅助音*式不协和，或是以共同音进入的*延留音*式不协和。其中延留音类型更受青睐。
...
一旦引入了功能性不协和音，当功能变化时，它必须在同一音部中向下级进解决。该不协和音在解决之前也可以被*转移*到另一音部——例如，若有一连串多个和弦呈现同一功能，出现在女低音中的不协和音可以在下一个和弦中转移到男高音，随后在功能变化时于男高音中解决。（在内声部之间、或从内声部向外声部转移不协和音，比从外声部向内声部转移更典型，听起来也更平滑。一旦不协和音出现在旋律或低音中——那里更易被察觉——它往往就在该音部中解决。）
...
功能性不协和音的解决常常与其他声部进行原则发生冲突。除 *schemata*（程式：足够常见、因而听起来得体的标准模式，即便它们遵循不同的规则）之类的特殊情况外，功能性不协和音的解决优先于其他原则，如*最短路径法则*、与低音的反向进行，以及宁取共同音与级进而不取旋律跳进。不协和音的解决绝不是非法平行进行的借口，且极少会导致非标准的重复音。
