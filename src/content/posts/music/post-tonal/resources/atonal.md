---
title: "Analyzing atonal music｜分析无调性音乐"
published: 2022-03-27
description: "无调性音乐的分析方法，含音级集合、音列与中心性判断。"
cover: /media/music/graphics/postTonal/clockFace.png
...
sourceLink: https://openmusictheory.github.io/atonal.html
comment: true
draft: false
---

> **来源**：本文选自 [Open Music Theory](https://openmusictheory.github.io/atonal.html)，由 Hybrid Pedagogy Publishing 出版，采用 [CC BY-SA 4.0](https://creativecommons.org/licenses/by-sa/4.0/) 许可。
>
> **中文名**：分析无调性音乐　|　**归档说明**：中英对照，每段英文原文后附中文译文，英文原文未作删减；图片与音频已本地化，站内链接已改为本站链接。
...
以下是一些提示、术语与常见结构，对分析二十世纪初的无调性音乐——例如第二维也纳乐派成员（勋伯格、贝尔格、韦伯恩）所创作的音乐——会有所帮助。

## Intervals｜音程

Always use *chromatic intervals* rather than diatonic intervals when analyzing and comparing pitch materials. (See [Intervals and dyads](https://openmusictheory.github.io/Intervals.html) resource.)

在分析与比较音高材料时，始终使用*半音音程*而非自然音程。（参见[音程与二音组](https://openmusictheory.github.io/Intervals.html)一节。）

## The 12-pitch-class cycle — clock face｜十二音级循环——钟面
...
无调性作曲家常常从高度组织化的音级集合出发，然后把各音级分布到相差悬殊的各个音区。因此，为了找出哪些动机、和弦与片段属于同一类别，把音区作为一个因素排除掉是有益的。为此，取一个标有十二个音级的钟面（记谱写法无关紧要）

![钟面](/media/music/graphics/postTonal/clockFace.png)

and mark all the pitch classes involved in the fragment. For example, here is a C-major scale:

并标出该片段所涉及的全部音级。例如，下面是 C 大调音阶：

![钟面上的自然音阶](/media/music/graphics/postTonal/clockFace-diatonic.png)
...
这样一来，无论这些音级在总谱上如何分布于各个音区与各件乐器，都能容易地找出一组音级中的音程型态。

## Common pitch-class collections｜常见音级音列

**Diatonic** – The usual major and *natural*-minor collections (white key collection and all of its transpositions).

**Diatonic（自然音）** – 通常的大调与*自然*小调音列（白键音列及其所有移位）。

![大调音列](/media/music/graphics/postTonal/major.png)
![小调音列](/media/music/graphics/postTonal/minor.png)

**Pentatonic** – The black key collection and all of its transpositions, prime form: (02479).

**Pentatonic（五声）** – 黑键音列及其所有移位，本原形式：(02479)。

![黑键五声音列](/media/music/graphics/postTonal/pentatonicBlack.png)
![白键五声音列](/media/music/graphics/postTonal/pentatonicWhite.png)

**Octatonic** – An eight-pitch-class scale that alternates i1 and i2 (semitones and whole-tones).

**Octatonic（八声）** – 一种在 i1 与 i2（半音与全音）之间交替的八音级音阶。

![八声音阶](/media/music/graphics/postTonal/octatonic.png)

**Hexatonic** – A six-pitch-class scale that alternates i1 and i3 (semitones and minor thirds/augmented seconds).

**Hexatonic（六声）** – 一种在 i1 与 i3（半音与小三度／增二度）之间交替的六音级音阶。

![六声音阶](/media/music/graphics/postTonal/hexatonic.png)

**X-cell** – The chromatic tetrachord, or a four-pitch-class chromatic cluster, prime form: (0123).

**X-cell（X 细胞）** – 半音四音组，或一个四音级的半音音簇，本原形式：(0123)。

![X 细胞](/media/music/graphics/postTonal/xCell.png)
...
**Y-cell（Y 细胞）** – 全音四音组，或一个四音级的全音音簇，本原形式：(0246)。它是全音音阶的子集。

![Y 细胞](/media/music/graphics/postTonal/yCell.png)
...
**Z-cell（Z 细胞）** – 一种在 i1 与 i5（半音与纯四度）之间交替的四音级音列，本原形式：(0167)。相距小三度的两个 Z 细胞构成一个八声音阶。

![Z 细胞](/media/music/graphics/postTonal/zCell.png)
...
**French-sixth（法国增六）** – 通常的法国增六和弦音列，但剥去了其调性功能，本原形式：(0268)。它是全音音列与八声音列的子集。与 Z 细胞一样，相距小三度的两个法国增六和弦构成一个八声音阶。

![法国增六音列](/media/music/graphics/postTonal/frenchSixth.png)

**Whole-tone scale** – A six-pitch-class scale made up of successive whole tones, prime form: (02468T).

**Whole-tone scale（全音音阶）** – 由连续全音构成的六音级音阶，本原形式：(02468T)。

![全音音阶](/media/music/graphics/postTonal/wholeTone.png)
...
**Acoustic scale（音响音阶）** – 一种七个音级的音阶，近似大调音阶，但把 *fa* 升高为 *fi*、把 *ti* 降低为 *te*，以对应自然泛音列中的第七与第十一分音。

![音响音阶](/media/music/graphics/postTonal/acoustic.png)
...
**Dual-diatonic（双重自然音）** – 由相隔五度的两个自然音音列取并集而成的八音级音阶——例如 C 大调与 G 大调：C – D – E – F – F# – G – A – B。它包含两个自然音音阶、两个 Z 细胞与两个 Y 细胞。主要由巴托克使用。

![双重自然音（fa–fi 型）](/media/music/graphics/postTonal/dualDiatonicFaFi.png)
![双重自然音（te–ti 型）](/media/music/graphics/postTonal/dualDiatonicTeTi.png)
...
音级移位所做的是同一件事。不过，由于音级的循环性质，音级移位同时也对应于音级音列在钟面上的*旋转*。
...
移位操作可以用大写字母“T”加下标表示，下标指明移位的半音数。对音级移位，使用有序音级音程（数字 0–11）。把一个音列或片段向上移高一个全音标记为 **T2**；向上三全音为 **T6**，向下半音为 **T11**（mod12(–1) = 11）。
...
*旋律倒影*发生于把一条旋律中所有上行的旋律音程替换为大小相同的下行音程，并把所有下行音程替换为大小相同的上行音程之时。下面两条旋律互为倒影。

![上行旋律](/media/music/graphics/postTonal/melodyUp.png)
![倒影后的旋律](/media/music/graphics/postTonal/melodyDown.png)
...
*音高倒影*发生于所有音高围绕音高空间中的一条对称轴翻转之时（换言之，对称轴是一个音高）。在上面那条旋律的例子中，轴是 G4。由于第一个音高就是 G4，它保持不变。下一个音高（A4）在 G4 之上两个半音；它被替换为在 G4 之下两个半音的 F4。B4（G4 之上 i4）被替换为 E-flat4（G4 之下 i4）。以此类推。

Pitch inversion can apply to a melody, but also to a simultaneity, or to a collection that a passage is based on.

音高倒影可以应用于旋律，也可以应用于同时发声音响，或应用于某一段落所依据的音列。
...
一般而言，当两条旋律、两个同时发声音响或两个音列可以通过音高倒影相关联时，它们在某种意义上被视为等同（但显然并不相同）。
...
*音级倒影*发生于一个音列的所有音级围绕音级空间中的一条对称轴翻转之时（换言之，对称轴是一个音级）。由于音级空间是循环的（向上走 12 个半音就回到出发点），对称轴由相距三全音的*两个*音级构成。这一点在钟面上看得最清楚。

![钟面上的四音组](/media/music/graphics/postTonal/clockFace-tetraChord.png)

When the above set is inverted around the axis C/F-sharp, it becomes the following set:

把上述集合围绕 C／F-sharp 轴作倒影，它便成为下面的集合：

![钟面上倒影后的四音组](/media/music/graphics/postTonal/clockFace-tetraChordInverted.png)
...
当音级用数字标记时（C = 0，C-sharp = 1，……B = 11），音级倒影可以通过确定一个*和数*、再求该和数与原音列中每个音级之差来计算。所得之差即为新音列的音级。
...
在上面的例子中，集合 {C, D, E, F} 即 {0, 2, 4, 5}。其倒影 {G, G-sharp, B-flat, C} 为 {7, 8, 10, 0}。围绕 C／F-sharp 轴对原集合作倒影时，C 变为 C（0 变为 0）、D–B-flat（2–10）、E–G-sharp（4–8）、F–G（5–7）。所有这些音级对相加均为 0 或 12，而二者在模 12 意义下等同。因此倒影和数为 0，该倒影标记为 **I0**。
...
围绕 C-sharp／G 轴作倒影是 **I2**。围绕 D／G-sharp 轴作倒影是 **I4**。围绕 E-flat／A 轴作倒影是 **I6**。围绕 E／B-flat 轴作倒影是 **I8**。围绕 F／B 轴作倒影是 **I10**。和数为奇数时，轴会落在音级之间（例如 **I1** 落在 C／C-sharp 与 F-sharp／G 之间）。
...
一些无调性作曲家喜欢使用能够倒影和／或移位到自身的集合。能够在围绕对称轴倒影时不改变任何音级的集合称为*倒影对称*。能够在按某个音程移位时不改变任何音级的集合称为*移位对称*或*旋转对称*。Z 细胞与法国增六音列都是既旋转对称又倒影对称的集合。
