...
sourceLink: https://openmusictheory.github.io/atonalGlossary.html
comment: true
draft: false
---
...
**collection（音列）** – 把多个音级作为单一实体看待的通用术语。集合、集合类、音阶、同时发声音响、和弦与音程都是音列的具体种类。
...
**interval class（音程级）** – 两个*音级*之间按钟面最短距离计算的半音数。例如，C 与 E 构成音程级 4。无论哪一个*音高*更高或更低，情况总是如此，因为音程级只涉及*音级*。音程级标记为 **ic1**、**ic2**……**ic6**。（没有比 **ic1** 更小或比 **ic6** 更大的音程级。）
...
**interval vector（音程向量）** – 集合类的音程向量描述一个集合类中出现的所有音程级。音程级共有六个（1–6）。音程向量用尖括号按 1 到 6 的顺序给出各音程的数量。音程向量 **<101102>** 表示该集合有一个 ic1（小二度／大七度）、没有 ic2、一个 ic3（小三度／大六度）、一个 ic4（大三度／小六度）、没有 ic5（纯四度），以及两个 ic6（三全音）。
...
**ordered pitch interval（有序音高音程）** – 从一个音高（不是音级）到下一个音高的半音数。上行音程用正数表示，下行音程用负数表示。例如：B4–G5 的有序音高音程为 8（上行八个半音）。B3–G5 为 20。B4–G4 为 –4。
...
**ordered pitch-class interval（有序音级音程）** – 从一个音级到另一个音级的*上行*半音数。G–B 为四个半音，有序 PC 音程为 **4**。B–G 为上行八个半音，有序 PC 音程为 **8**。
...
有序音级音程也是有序音高音程的模 12 形式。例如，B4–G4 为 –4 个半音（下行 4 个半音）。mod12(–4) = 8。C3–D4 为 14 个半音。mod12(14) = 2。

**pitch** – A pitch class in a specific register, such as C4 (middle C).

**pitch（音高）** – 处于特定音区中的音级，例如 C4（中央 C）。
...
**pitch class（音级）** – 半音阶上十二个级位之一，用音名（C、D-sharp、B-flat 等）或数字 0–11 表示（C = 0，C-sharp = 1，……B = 11）。在无调性音乐中，记谱写法除了便于演奏之外很少重要，因此等音音级被视为相同（C = B-sharp = D-double-flat = 0）。

**pitch-class set** – An *unordered* collection of pitch classes, usually grouped into curly brackets: {C, E, G}, {D, E-flat, G}, or {4, 5, 9}.

**pitch-class set（音级集合）** – 一种*无序的*音级音列，通常用花括号括起：{C, E, G}、{D, E-flat, G} 或 {4, 5, 9}。
...
**pitch-class set class（音级集合类）** 或简称 **set class（集合类）** – 一类彼此通过移位或倒影相关联的音级集合。例如，12 个大三和弦都通过移位相关联。虽然每个大三和弦都是不同的音级集合，但它们都属于同一个集合类（同一类集合）。注意小三和弦是倒过来的大三和弦（小三度–大三度，而非大三度–小三度）。因此，由于大三和弦与小三和弦可以通过倒影相关联，它们属于同一个集合类。集合类通常按其*本原形式*命名（参见本术语表中的*本原形式*）。
...
**prime form（本原形式）** – 由于一个集合类最多有 24 种不同形式（12 个移位乘以 2 个倒影），为便于分类，选取其中一种形式作为其名称或参照形式。那种形式就是本原形式。简而言之，本原形式是该集合类中使音级最紧密地排列在 C（0）及其之上的一种倒影与旋转形式。

For help finding the prime form of a set, Jay Tomlin’s [set theory calculator](http://www.jaytomlin.com/music/settheory/) can be helpful. The following video demonstrates how to use it.

若需帮助寻找某个集合的本原形式，Jay Tomlin 的[集合理论计算器](http://www.jaytomlin.com/music/settheory/)会很有用。下面的视频演示了它的用法。

<iframe src="http://player.vimeo.com/video/63210462" width="100%" height="400" frameborder="0" allow="autoplay; fullscreen; picture-in-picture" allowfullscreen></iframe>

[SetTheoryCalculator](http://vimeo.com/63210462) from [Kris Shaffer](http://vimeo.com/user11692346) on [Vimeo](http://vimeo.com).

[SetTheoryCalculator](http://vimeo.com/63210462)，作者 [Kris Shaffer](http://vimeo.com/user11692346)，发布于 [Vimeo](http://vimeo.com)。
...
**simultaneity（同时发声音响）** – 任何多于一个音高（音级）同时发响的音列。它包括二音组／音程、和弦、音簇，以及对位织体中切出的“萨拉米切片”。
...
**unordered pitch-class interval（无序音级音程）** – 一种普通的*单*半音音程：两个音高之间的半音数。复音程（大于一个八度）通常还原为相应的单音程。它们用小写 **i** 标记：例如 **i4** 是大三度。
