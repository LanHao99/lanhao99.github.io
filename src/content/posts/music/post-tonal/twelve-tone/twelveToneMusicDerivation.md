...
description: "由基本集合派生出整个序列的方法与实例。"
cover: /media/music/graphics/postTonal/derivedRow.png
...
sourceLink: https://openmusictheory.github.io/twelveToneMusicDerivation.html
comment: true
draft: false
---
*Twelve-Tone Music — Derived Rows*
...
*派生序列行*是指那些不重叠的片段（*离散*片段）属于同一集合类的序列行。由于不得重叠，离散子集把序列行分为六个 2 音片段、四个 3 音片段、三个 4 音片段或两个 6 音片段。（一般而言，我们主要关注以三音组或四音组派生的序列行。）
...
下面给出一例。这条序列行（出自韦伯恩《弦乐四重奏》Op. 28）被划分为离散的四音组与离散的二音组。所有四音组都属于集合类 (0123)。因此我们说，这条序列行是以四音组派生的，并由 (0123) *生成*。此外，离散的二音组表明了一种有趣的二音组派生，由 (01) 生成。

[![derivedRow](/media/music/graphics/postTonal/derivedRow.png)](/media/music/graphics/postTonal/derivedRow.png)
...
从作曲与听者导向的角度看，派生序列行极具启发性。由于序列行在移位、倒影等操作下其集合类内容不会改变，即使使用不同的序列行形式，这些集合类也会在全曲中持续循环出现。因此，派生序列行保证了极少数几个集合类的规律性重现——从而确保全曲具有某种统一的特质。
