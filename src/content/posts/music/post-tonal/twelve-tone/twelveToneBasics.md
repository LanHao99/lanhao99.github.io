...
sourceLink: https://openmusictheory.github.io/twelveToneBasics.html
comment: true
draft: false
---
...
十二音音乐最常与一种称为*序列主义*的作曲技法或风格联系在一起。然而这两个术语并不等同。*序列主义*是一个宽泛的称谓，指对事物——无论是音高、时值、力度等等——的*排序*。十二音作曲则更具体地指以*十二个音级*的各种排序为基础的音乐。
...
这种作曲风格最常与一个作曲家群体联系在一起，其领袖是阿诺德·勋伯格，成员还包括极具影响力的作曲家安东·韦伯恩与阿尔班·贝尔格。但十二音作曲技法以及与之相关的观念对许多伟大作曲家都产生了深远影响，序列音乐与十二音音乐至今仍有人在写作。这类音乐大多共享如下所述的相似公理，但作曲家们利用这些基本观念发展出了完全原创的路径。
...
十二音音乐以*序列*（有时称为*序列行*）为基础，序列按特定顺序包含全部十二个音级。并不存在供所有十二音音乐使用的唯一序列；多数作曲家为每部作品写出独特的序列行。（十二音序列共有 12!——即 12 的阶乘——种，等于 479,001,600 个独特的序列行形式。可能性相当多！）下面是一例，即韦伯恩《钢琴变奏曲》Op. 27 的序列行：

[![basicRow](/media/music/graphics/postTonal/basicRow.png)](https://openmusictheory.github.io/Graphics/form/basicRow.png)

There are some general rules for using a twelve-tone row, though as I said, individual approaches are always different:

使用十二音序列行有一些通则，不过正如我所说，具体的处理方式总是各不相同：
...
十二音序列行既可用作主题，也可用作动机的来源。和弦可以从序列行中派生，序列行也可同时用于主题与和声的目的。我们把上方为 Op. 27 所示的基本排序称为*原型*（P）。由于它从 B 开始（音级 11），我们将其标记为 **P11**。
...
序列行可以移位、倒影、逆行，或这些操作的任意组合。对原型作倒影便得到“I 形式”。与 P 形式一样，I 形式按其第一个音级命名。因此，下面这一行是上一行的倒影，称为 **I0**。注意它从 C（0）开始。

[![inversion](/media/music/graphics/postTonal/inversion.png)](https://openmusictheory.github.io/Graphics/form/inversion.png)
...
原型与倒影形式也可以反向演奏，这称为逆行。在下面的例子中，请注意它与上面的 P11 与 I0 行之间是如何对应的。当 P 形式被逆行时，我们称之为“R 形式”。当 I 形式被逆行时，则称为“RI 形式”。如例所示，R 形式与 RI 形式按其*最后一个音级*命名。

[![family](/media/music/graphics/postTonal/family.png)](https://openmusictheory.github.io/Graphics/form/family.png)
...
该图只显示了四种序列行形式，但每一种形式都有十二个移位。因此，单条序列行共繁衍出 48 个序列行：12 × 4。这些序列行的集合称为*序列行类*，作曲家在写作时正是从*序列行类*中取材。
