...
description: "音高集合移位的记号与运算，以及移位下的音程测量。"
cover: /media/music/graphics/postTonal/transposition.png
...
sourceLink: https://openmusictheory.github.io/transposition.html
comment: true
draft: false
---
*Transposition*
...
在后调性音乐中，移位常常与运动相联系：取一个和弦、动机或旋律，当它被移位时，听觉效果就是该和弦、动机或旋律朝某个方向*移动*。德彪西《沉没的教堂》中两个彼此不相连的片段便具有这种效果：

[![移位](/media/music/graphics/postTonal/transposition.png)](/media/music/graphics/postTonal/transposition.png)
...
开头的动机——由 B、D、E 三个音，即 {11, 2, 4} 构成——在第 18 小节被向上移位四个半音，表现大教堂缓缓升出水面。移位保存了一个对象的音程内容，不仅如此，它还保存了该对象音程的具体排列方式。当我们听到上面第 18 小节的那一片段时，我们之所以能识别出它与第 1 小节片段之间的关系，正是因为同样的音程再次出现，只是起始音高不同。
...
移位是一种操作——即对某个音高、音级或由这些事物构成的集合*所做*的事情——或者也可以是一种*测量*——表示事物之间的距离。我们用 *Tn* 来表示它，其中 *n* 表示两个事物之间的有序音级音程。要把某事物移位 *Tn*，就在该事物的每一个元素上加 *n*（模 12）。给定上面第 1 小节的音级集合，并作 *T4* 移位：

[![T4 移位](/media/music/graphics/postTonal/t4.png)](/media/music/graphics/postTonal/t4.png)

The result is the pitch classes in m. 18. *T4* {11, 2, 4} = {3, 6, 8}.

结果便是第 18 小节的音级。*T4* {11, 2, 4} = {3, 6, 8}。
...
另一方面，要确定两个事物*之间*的移位关系，就用第二个事物减去第一个事物。如果所得的数字全都相同，那么这两个事物便由该 *Tn* 相联系。

[![移位关系的测量](/media/music/graphics/postTonal/t4Measurement.png)](/media/music/graphics/postTonal/t4Measurement.png)
...
上面音乐谱例中 *T4* 的箭头标记便是这样得出的——用第 18 小节的音级整数“减去”第 1 小节的音级整数。
