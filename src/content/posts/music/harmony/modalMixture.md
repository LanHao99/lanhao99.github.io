...
sourceLink: https://openmusictheory.github.io/modalMixture.html
comment: true
draft: false
---
...
*调式交替*（也称*调式借用*）指使用属于同主音调（平行调）的和弦——例如，F 大调的一个段落中纳入一个或多个来自 f 小调的和弦。请注意，同借用和弦的使用一样，这并不必然构成转调。只有终止式才能确认新调。若在新调中没有终止式，这些非自然音和弦只是“借来的”。
...
请注意，在小调中用导音代替下主音，或使用旋律小调的音型（*sol–la–ti–do*），并*不*构成调式交替。这些被视为小调调式的“固有”内容。

## Roman numeral notation｜罗马数字标记法
...
当所借和弦的根音属于主调时（例如用 e 小三和弦代替 E 大三和弦），罗马数字保持不变，因为罗马数字只表示和弦根音的音级。若和弦性质体现在罗马数字中，则须作出调整，以确保所借和弦的性质得到体现。例如，若一首小调作品以*皮卡迪三度*（大三和弦形式的主三和弦）结束，其罗马数字是 **I** 而不是 **i**。（数字低音也将随之改变，以体现这一半音变化。）
...
若根音相对于主调被改变，则在罗马数字前使用降号或升号表示这一变化：降号表示*降低*（即比正常低一个半音），升号表示*升高*（比正常高一个半音）。例如，大调中的 *le–do–me* 和弦是 **♭VI**。（同样，按需修改数字低音。）

## Functional bass notation｜功能低音标记法
...
从平行调借来的和弦属于半音变化和弦，因此其功能低音符号应置于方括号内。例如，若大调中的 **S4** 和弦借自平行小调（*fa–le–do* 而非 *fa–la–do*），其功能低音符号是 **[S4]** 而不是 **S4**。若低音未发生变化，这是功能低音唯一需要修改之处（但务必同时修改数字低音标记）。
...
若低音发生半音变化，则必须在功能低音中于数字前用加号或减号体现出来（同时使用方括号）。例如，若一个大调段落中包含建立在 *le* 上的 5/3 和弦（*le–do–me* 而非 *la–do–mi*），其功能低音为 **[Tx–6]**。
