...
sourceLink: https://openmusictheory.github.io/harmonicSyntax1.html
comment: true
draft: false
---
...
*和声句法*关注的是和声（和弦）依照何种规范或原则被置入有意义的连续进行之中。这些规范包括各类进行相对常见或相对少见。这些规范为熟悉该风格的听者生成期待：若 **IV–V** 比 **IV–VI** 更常见，那么 **IV** 和弦的出现就会令听者期待下一个和弦更可能是 **V** 而非 **VI**。
...
在西方古典音乐中，和声总体上归入三大*和声功能*——主功能（T）、下属功能（S）与属功能（D）——这些功能把与其他和弦进行关系相似的和弦归为一组。例如，由于 **II** 与 **IV** 都是下属和弦，它们会参与许多同类的和弦进行，有时还可以相互替换，而只对音乐效果造成极小改变。
...
在局部层面（和弦与和弦之间的进行），我们可以用 **T–S–D–T** 这一循环来概括这些功能的倾向。也就是说，和声倾向于按这三个功能构成的循环进行：
...
这并不排除 T 进行到 D、D 进行到 S 等情况。但它确实意味着这些进行相对少见，至少在古典音乐中如此。
...
更高层次的音乐结构也会影响这些和声功能进行所依循的规范。目前我们只考虑一种影响和弦进行倾向的更高层结构——乐句——并把研究范围限定在孤立的、完整的、自足的乐句上。这是一种理想化、过度简化的设定——如同严格声部进行——对学习基础知识很有用。有些这样的乐句甚至真的存在于音乐作品中！但大多数时候，有诸多相互竞争的因素共同影响作曲家在某一时刻所采用的和弦进行策略。不过，理想化乐句是一个有用的起点。后续学习将探究古典作曲家如何在更大型的音乐作品中运用和声进行，把多个（并不自足的）乐句组合成更大的主题与乐章。

## The idealized phrase｜理想化乐句
...
*理想化乐句*（也称*乐句模型*）是单个音乐乐句，它完整地经历一轮和声功能的循环，起于主功能并终于主功能。（严格声部进行练习就是这样的乐句。）这类乐句从一个稳定点（主功能）开始，离开该稳定点，最终导向一个高度紧张并得到解决的节点（*正格终止*）。这种稳定—不稳定—稳定的模式，或静—动—静的模式，在结尾处只有一个目标，这在分类对位与严格键盘风格声部进行中都应该很熟悉。（这一模式也支配着古典音乐中大规模曲式结构。）
...
呈现这一完整和声循环的最简单乐句是主—属—主进行：**I–V–I**。这一乐句以最稳定的和声（**I**）开始并结束，并包含一个*正格终止*（**V–I**）。**V** 是不稳定性的高点，含有最强力指向最终到达音（*do*，即主音）的倾向音（*ti*）。
...
这一和声循环可以通过插入下属和弦、失稳主和弦或两者来扩展，如下列各例所示：
...
然而，乐句很少只有 3–5 个和弦，而一个和声功能也可以由不止一个和弦来表达。因此我们可以把这些和声功能不仅仅理解为和弦，而理解为乐句中长度不等的*区域*，这些区域可以由若干和弦或短短的和弦进行构成。更一般地说，我们的理想化音乐乐句包含一次功能区域的进行 **T → (S) → D → T**，以 **T1** 开始，以正格终止（**D5–T1**）结束，如下例所示。

## Triggering and prolonging harmonic functions in an idealized phrase｜理想化乐句中和声功能的触发与延伸
...
为了确立（即触发）一个和声功能区域，作曲家倾向于使用*低音上固定的音级*。换言之，主功能倾向于由 **T1** 触发（总是 **I**），下属功能由 **S2** 或 **S4** 触发（包括各种 **II** 与 **IV** 和弦，无论原位或转位，无论带七音或不带七音），属功能由 **D5** 触发（**V**，带七音或不带七音，或者一个[*复合终止*](/posts/music/harmony/cadenceTypes/)）。这四类和弦——**T1**、**S2**、**S4** 与 **D5**——称为*功能性和弦*（因为它们触发功能）或*终止性和弦*（因为它们可以参与终止式）。
...
其他和弦常称为*对位性和弦*或*装饰性和弦*，通常用于在整个区域中*延伸*某一功能。
...
在和声分析中，功能延伸的表示方法是：在各个和弦标记（罗马数字或功能低音）下方书写或键入 T、S 或 D，并从功能区域的起点画一条线延伸到终点。
...
下面一段摘自莫扎特《A 大调钢琴奏鸣曲》K. 331 第一乐章第 1–4 小节，原谱之下附有和声缩编与分析。这样的分析称为*解释性*和声分析，因为各和声是按其在该乐句中行为的方式加以解释，而不仅仅是标注。在这一乐句中，请注意以下几点：

- The tonic zone is triggered by a root-position tonic triad (**I** or **T1**).／主功能区域由原位主三和弦（**I** 或 **T1**）触发。
- Contrapuntal dominant chords (**D7** — first-inversion dominant chords) create a passing bass motion between the opening **I** chord, the **vi** in m. 3, and the return of **I** in m. 4.／对位性属和弦（**D7**——第一转位的属和弦）在开头的 **I** 和弦、第 3 小节的 **vi** 与第 4 小节 **I** 的回归之间构成经过性的低音运动。
- The [cadential progression](/posts/music/harmony/harmonicAnalysis/) begins in m. 4 with the move from **I** to **ii6** (**S4**) and then to the cadential six-four and dominant triad (**D5**). Note that the entirety of the cadential progression in m. 4 is made up of *cadential chords* — chords with fixed scale degrees in the bass.／[终止进行](/posts/music/harmony/harmonicAnalysis/)从第 4 小节开始，由 **I** 移动到 **ii6**（**S4**），再到终止四六和弦与属三和弦（**D5**）。注意第 4 小节中整个终止进行都由*终止性和弦*构成——即低音音级固定的和弦。
- In contrast, the entire tonic-prolongation zone is made up of *contrapuntal chords* — variable scale degrees in the bass — with the exception of the **I** chord that triggered the tonic function.／与之相对，整个主功能延伸区域都由*对位性和弦*构成——即低音音级可变——唯一的例外是触发主功能的那个 **I** 和弦。
- The **vi** chord is a root-position chord, but still an embellishing chord, while the **ii6** is an inverted chord, but still a functional/cadential chord. The difference is not the inversion, but the scale degree of the bass.／**vi** 和弦是原位和弦，但仍是装饰性和弦；而 **ii6** 是转位和弦，却仍是功能性／终止性和弦。区别不在于转位，而在于低音的音级。

[![k331 缩编](/media/music/graphics/harmony/k331-reduction.png)](/media/music/graphics/harmony/k331-reduction.png)

<iframe style="border-radius:12px" src="https://open.spotify.com/embed/track/5yfZxPrcN0hjwzU1OKIsdQ" width="100%" height="152" frameborder="0" allow="autoplay; clipboard-write; encrypted-media; fullscreen; picture-in-picture" loading="lazy"></iframe>

Not all classical phrases as neatly fit the general trends outlined in this resource. As discussed in [Style and tendency](/posts/music/voice-leading/four-voice/tendency/), the principles of harmonic syntax are both reliable and bendable/breakable, and it is often the music that bends/breaks the “rules” in interesting ways that we care about the most. So in your own analyses, keep these principles in mind as *general* principles, and simultaneously look for where composers meet these expectations as well as where they break them.

并非所有古典乐句都如此整齐地符合本材料所概述的一般趋势。正如 [Style and tendency｜风格与倾向](/posts/music/voice-leading/four-voice/tendency/) 中所讨论的，和声句法的原则既可靠，又可被弯曲／打破；而最让我们关注的，往往正是那些以有趣方式弯曲／打破“规则”的音乐。因此在你自己的分析中，请把这些原则视为*一般性*原则，同时留意作曲家在哪里符合了这些期待，又在哪里打破了它们。

For more details on the triggering and prolonging of harmonic functions in a classical phrase, see [Harmonic syntax – prolongation](/posts/music/harmony/harmonicSyntax2/).

关于古典乐句中和声功能之触发与延伸的更多细节，参见 [Harmonic syntax – prolongation｜和声句法：延伸](/posts/music/harmony/harmonicSyntax2/)。
