...
description: "功能和声的基本思路，以及功能听辨的训练方法。"
...
sourceLink: https://openmusictheory.github.io/functions.html
comment: true
draft: false
---
...
音乐*功能*描述某一音乐元素在构建更大音乐单位时所扮演的角色。功能与期待的观念紧密相关：在特定语境中给定某一元素，接下来可能出现的是什么元素？同样地，某一元素如何满足或违背由它之前的内容所设定的期待？
...
音乐功能通常有两个界定性特征：倾向于归属于该功能的音乐元素所具有的特征（例如和弦中通常出现哪些音），以及在音乐元素的连续进行中倾向于出现在它之前或之后的元素（或功能）种类。请注意，这完全取决于某一音乐*风格*的典型模式。不同的音乐风格可能表现出不同的功能，或同一功能的不同行为方式。对功能的研究与对风格的研究密不可分。
...
在西方艺术音乐与流行音乐中，最常就其功能属性加以研究的两种音乐特征是*和声*与*曲式*。对和声功能与曲式功能的研究，将导向对和声*句法*与曲式*句法*的理解：即音乐元素依照何种规范或原则被组合成有意义的、风格上得体的连续进行。因此，对和声或曲式的研究并不是学会正确地标注和弦、乐句和模块，而是*解释和弦、乐句、模块等在它们所处的更大语境中所扮演的角色*。这当然要求能够流利地识别（从而标注）单个音乐元素。但识别只是更大、更有趣的分析过程的起点。正是这种分析工作，才能带来对所分析的音乐作品及它们所属风格的真正理解。
