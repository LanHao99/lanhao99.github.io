...
sourceLink: https://openmusictheory.github.io/appliedChords.html
comment: true
draft: false
---
...
*主音化*是通过使用从某和弦为其主音的那个调中借来的和弦，暂时强调一个非主音和弦的过程。与*转调*不同，主音化在新调中没有终止式，而只是短短一串从另一个调借来的和弦。
...
被主音化的和弦通常是属于当前调的和弦。用来强调它的和弦通常是从另一个调借来的和弦，而这些和弦通常又是当前调固有和弦的半音变化形式。
...
最直接的例子是把下属和弦进行半音变化，将 *fa* 改为 *fi*，然后照常进行到属和弦。把 *fa* 改为 *fi* 的这一变化，使一个普通的下属和弦变成了在“属调”中具有属功能的和弦。
...
例如，取 C 大调中的和弦进行 **F–G–C**。用罗马数字我们会把这个进行标为 **IV–V–I**。若把 F 和弦中的 *fa*（F）改为 *fi*（F#），就得到 **F#dim–G–C**。在 C 调中，我们也许会把这一进行分析为 **#ivº V I**，并指出根音与性质的变化。然而，我们也可以注意到 F#dim 是 G 大调固有的和弦；它是 G 调中具有属功能的和弦（**viiº**）——而 G 调正是后一个和弦为其主音的那个调。换言之，我们从 G 调借来属和弦，并把它*应用*于 G 大三和弦。因此，我们可以把 F#dim 重新解释为 G 和弦的*副属和弦*，标记为 **viiº/V**——读作“五级的七级”。于是这一进行就标为 **viiº/V V I**。
...
不过要注意，虽然我们“借来”或“应用”了 G 大调的属和弦，该和弦仍是在 C 大调的语境中起作用。它的音级是 *fi*、*la*、*do*：这是对音级的变化，而它们显然表达下属功能。因此在 C 大调中，进行 **F#dim–G–C** 表达的仍是功能进行 **S–D–T**。（在功能低音中——见下文——这一进行将分析为 **[S+4] D5 T1**。）
...
*练习：改变下面段落中的一个音，把下属和弦变为借用和弦。你会给该和弦标注怎样的罗马数字与功能低音？*

<iframe src="https://trinket.io/embed/music/c6b5004279" width="100%" height="400" frameborder="0" allowfullscreen></iframe>
...
这种和弦借用可以发生于*主调中的任何大三和弦或小三和弦*。任何自然音三和弦都可以接受一个借用和弦——即一个经过半音变化的和弦，它同时作为属和弦发挥作用，而其后继和弦为其主音。因此，例中的 **F#dim–G** 进行可以出现在 G 大三和弦所属的任何调中：G 大调、C 大调、D 大调、b 小调、a 小调或 e 小调。

Two things will always be true of the applied chord:

借用和弦总是具备以下两点：

- The chromatically altered chord will function as a dominant chord in the key of the chord that follows it (**V** or **VII**).／该半音变化和弦将在其后继和弦所属的调中起属和弦的作用（**V** 或 **VII**）。
- The chromatically altered chord will be an alteration of the function that logically precedes the function of the chord that follows it.／该半音变化和弦是其后继和弦功能在逻辑上之前那个功能的变体。
...
就后一点而言：若被主音化的和弦在当前调中具有主功能（如 *mi–sol–ti* 或 *la–do–mi*），则借用和弦是当前调的属功能变体。若被主音化的和弦在当前调中具有属功能，则借用和弦是当前调的下属功能变体。若被主音化的和弦在当前调中具有下属功能，则借用和弦是当前调的主功能变体。

| applied chord: 借用和弦 |  | tonicized chord: 被主音化的和弦 |
| :---: | :---: | :---: |
| altered T | → | S |
| altered S | → | D |
| altered D | → | T |

## Analytical notation｜分析标记法
...
副属和弦总是使用*斜线标记法*。罗马数字中不写 **#IV**、**#V** 等，惯例是在它借自的那个调中表示其身份。若在降 E 调中，用一个 F 属七和弦来主音化一个降 B 三和弦，我们把 **F7** 和弦标为 **V7/V**，而不是 **II7**。它与自然音和弦的罗马数字写在同一行。
...
在斜线标记之下，我们标注它的和声功能（**T**、**S** 或 **D**）。这里展示的是该和弦在*主*调中起作用的方式，而不是它借自的那个调。因此在降 E 调中，**F7** 虽然可能是 **V/V**，但它是半音变化的 **S**，因此归于 **S** 功能区域。
...
这一区分很重要。借用和弦是在调性乐句的语境中使用的，而在新调中没有终止式（即没有转调），因此这些借来的和弦仍在原调内部发挥作用。所以，虽然我们用罗马数字中的斜线标记体现它们的“借用”性质，同样重要的是用*非*转调性的功能标记来体现它们在乐句更广阔和声语境中的作用。

Following is a short keyboard-style passage that includes two different applied chords, notated with Roman numerals.

下面是一段简短的键盘风格乐段，包含两个不同的借用和弦，以罗马数字标出。

<iframe src="https://trinket.io/embed/music/45c20d9624" width="100%" height="400" frameborder="0" allowfullscreen></iframe>

### Notes on functional-bass notation｜功能低音标记法说明
...
同 [chromatically altered subdominant chords｜半音变化的下属和弦](/posts/music/harmony/alteredSubdominants/) 一样，每个借用和弦的功能低音符号都包含两个要素。第一，在功能低音分析的常规一行上，是表示它在当前调中的功能以及低音音级的符号（低音音高有半音变化时加“+”或“–”），外加方括号以表明该和弦经过半音变化。（无论在哪个声部发生半音变化，方括号都是必需的。）第二，在功能低音分析的常规一行之下，是表示它借自哪个调、以及该和弦在那个调中将具有的功能低音符号。
...
在上面 C 调中 **F#dim–G–C** 的例子中，常规功能低音一行写作 **[S+4] D5 T1**，而在 **[S+4]** 之下是符号 **D7/V**。后一符号用斜线表示“在某调中”，用罗马数字表示该调的主音*相对于当前调*的位置。在研究转调时，我们将以类似方式使用罗马数字，表示音乐所转入的调性区域的主音。不过，在功能低音分析的语境中，罗马数字*从不*用来表示和弦的根音。
...
下面是上例，但用功能低音分析的上层取代了罗马数字。（Trinket 只支持一层歌词／和声分析。）哪些和弦需要第二层功能低音分析？那些符号应当是什么？

<iframe src="https://trinket.io/embed/music/9a8ff25f3c" width="100%" height="400" frameborder="0" allowfullscreen></iframe>

(You can find the interpreted functional bass version [here](https://trinket.io/music/406c96fec0).)

（你可以在 [here｜此处](https://trinket.io/music/406c96fec0) 找到解释性功能低音的版本。）

## Scale degrees in applied chords｜借用和弦中的音级
...
正如一个调中各种具有属功能的和弦会包含若干属功能音级的组合——*sol*、*ti*/*te*、*re*、*fa* 和/或 *la*/*le*（通常是 *le*）——每一类副属和弦也都有各自常用的音级集合。它们如下。

**D/II** — *di*, *mi*, *sol*, *la*, and *te*.／**D/II** —— *di*、*mi*、*sol*、*la*、*te*。
**D/lowered-III (minor)** — *re*, *fa*, *le*, and *te*.／**D/降 III（小调）** —— *re*、*fa*、*le*、*te*。
**D/III (major)** — *ri*, *fi*, *la*, *ti*, and *do*.／**D/III（大调）** —— *ri*、*fi*、*la*、*ti*、*do*。
**D/IV** — *mi*, *sol*, *te*, *do*, and *ra*.／**D/IV** —— *mi*、*sol*、*te*、*do*、*ra*。
**D/V** — *fi*, *la*, *do*, *re*, and *me*.／**D/V** —— *fi*、*la*、*do*、*re*、*me*。
**D/lowered-VI (minor)** — *sol*, *te*, *ra*, and *me*.／**D/降 VI（小调）** —— *sol*、*te*、*ra*、*me*。
**D/VI (major)** — *si*, *ti*, *re*, *mi*, and *fa*.／**D/VI（大调）** —— *si*、*ti*、*re*、*mi*、*fa*。
**D/lowered-VII (minor)** — *la*, *do*, *me*, and *fa*.／**D/降 VII（小调）** —— *la*、*do*、*me*、*fa*。

## Functional dissonances in applied chords｜借用和弦中的功能不协和音
...
由于主音化是从另一个调暂时借用一个和弦，而该和弦又由那个调的各个音级构成，因此借用和弦涉及*从那个调借来音级的倾向*。换言之，在借用和弦中，判断功能协和音（触发音与关联音）与不协和音，以及像 *ti* 这样的倾向音，都应相对于所借自的那个调，而不是主调。
...
作为一条捷径，这通常意味着七和弦的七音或减和弦的五音是功能不协和音。（在半减七和弦或全减七和弦中，五音与七音往往都是功能不协和音。）同样作为一条捷径，被升高半音的那个音往往是被主音化调的导音。
...
在功能低音中，利用斜线之前的功能标记与斜线之后的罗马数字，可以更可靠地判定功能不协和音。考虑 G 大调中的 **D5/V**（变化的 **S2**）和弦。在主调中，该和弦包含 *re*（A）、*fi*（升 C）、*la*（E）与 *do*（G）。但这些音高在 V 调（D 大调）中是 *sol*、*ti*、*re*、*fa*。因此升 C 应按导音对待，而 G（在 D 调中为 *fa*）则应按属功能的功能不协和音对待。
...
*练习：在上列各例中，找出每个借用和弦中的功能不协和音。它是哪个音高？它如何进行到下一个和弦？*
...
*练习：针对上面的项目列表，判断每个借用和弦中哪些唱名音节是功能不协和音。*
