...
sourceLink: https://openmusictheory.github.io/Modulation.html
comment: true
draft: false
---
...
*主音化*是指从另一个调借用和弦或一小串和弦，用以强调——即*主音化*——主调中的某个和弦。（参见[借用和弦的分析](/posts/music/harmony/appliedChords/)。）*转调*则指较长的一串和弦强调一个新的主音，从而使人感知到新调。主音化与转调的主要区别在于终止式的有无：主音化*不*在被主音化的调中引入终止式；转调*则*在新调中至少引入一个终止式（PAC、IAC 或 HC）。

There are several ways in which a composer can effect a modulation. The most common are described below.

作曲家实现转调有若干种方式。以下介绍最常见的几种。

## Direct/phrase modulation｜直接转调／乐句转调
...
*直接转调*指前一个调中的和弦之后直接紧跟新调中的和弦。换言之，两调之间没有平滑的过渡或重叠，而只是一次从一个调直接移动到下一个调。这种情况常发生在乐句边界处：旧调主和弦结束一个乐句，新调主和弦开始下一个乐句。当直接转调发生在乐句边界上时，也称*乐句转调*。
...
在小步舞曲或奏鸣曲中，呈示部结束与呈示部反复开始之间（若存在呈示部反复）的位置上，乐句转调的例子比比皆是。
...
在和声分析中，直接转调的标记方式是：在旧调的最后一个和弦之后写出新调、冒号，然后是位于新调的第一个和弦。
...
## Step-up/pump-up modulation｜升调转调／推高转调
...
在流行音乐文献中，歌曲接近结尾处常出现以全音或半音进行的直接转调。流行音乐中由旧调主音级进至新调主音的直接转调／乐句转调，也称*升调转调*或*推高转调*。升调转调的标记方式与直接转调相同。

“I Wanna Be Sedated” by the Ramones includes an obvious *step-up* modulation (1:10).

雷蒙斯乐队的《I Wanna Be Sedated》中含有一处明显的*升调转调*（1:10）。

<iframe style="border-radius:12px" src="https://open.spotify.com/embed/track/6vvmYYUvGXtZLU8msxKvzF" width="100%" height="152" frameborder="0" allow="autoplay; clipboard-write; encrypted-media; fullscreen; picture-in-picture" loading="lazy"></iframe>

## Truck-driver modulation｜卡车司机转调
...
*卡车司机*转调是一种直接转调，它从旧调（通常是主和弦）移动到新调的属和弦，以准备该主音的到来，在流行音乐中同样常见。这一名称（由 Walter Everett 创造）背后的想法是：音乐在进入“空挡”（新调属和弦）时短暂失去能量，随后进入更高的能量状态（新调主和弦，比旧调主音高一个级进）。卡车司机转调的标记方式与直接转调相同。

Billy Ocean’s “Get Outta My Dreams” contains a classic truck-driver modulation (3:55).

Billy Ocean 的《Get Outta My Dreams》中含有一处经典的卡车司机转调（3:55）。

<iframe style="border-radius:12px" src="https://open.spotify.com/embed/track/3v8vsQfMQio7ohYqFrEsaZ" width="100%" height="152" frameborder="0" allow="autoplay; clipboard-write; encrypted-media; fullscreen; picture-in-picture" loading="lazy"></iframe>

## Pivot-chord modulation｜中介和弦转调
...
*中介和弦*转调使用至少一个同时属于旧调与新调的和弦。它是共性写作时期调性音乐中最常见的转调类型。最平滑的中介和弦转调所使用的和弦在两调中表达相同的功能——常见的是下属功能，但其他功能安排也是可能且常用的。
...
当某个和弦在新调中表达属功能、而在旧调中是借用和弦时，它就不是中介和弦。此时该和弦起的是直接转调或卡车司机转调的作用。中介和弦必须同时属于两调的自然音级集合（须记住，在小调中 *la* 与 *le*、*ti* 与 *te* 都是小调的“固有”音级）。
...
中介和弦转调有特殊的标记方式。中介和弦照常先标出它在旧调中的分析符号。在该符号下方写出新调、冒号，以及该中介和弦在新调中的分析符号。使用记谱软件时，采用双层分析即可：用歌词工具创建多“段”和声分析，每调一段，在中介和弦处重叠。手写分析时，使用下例所示的方括号。

![中介和弦方括号标记法：d 小调的 VII 成为 F 大调的 V。](/media/music/graphics/harmony/pivot.png)
