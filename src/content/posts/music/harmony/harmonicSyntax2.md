...
sourceLink: https://openmusictheory.github.io/harmonicSyntax2.html
comment: true
draft: false
---
...
以下是在理想化古典乐句中延伸功能区域的主要技法。文中给出了具体进行的例子与记谱惯例。

## Change-of-figure prolongation｜数字低音变换式延伸
...
*数字低音变换式延伸*指低音重复（或保持，或下降一个八度），而上方的某一个或多个声部发生变化。功能保持不变（T/S/D），但罗马数字可能改变。

Examples include progressions like **V–V7** (both **D5**) or **IV–II6** (both **S4**).

例如 **V–V7**（两者都是 **D5**）或 **IV–II6**（两者都是 **S4**）这类进行。

## Change-of-bass prolongation｜低音变换式延伸
...
*低音变换式延伸*指同一功能的两个和弦紧接出现，但低音音级不同。某些情况下这是转位的变化：例如 **I–I6**。另一些情况下是根音的变化：例如 **I–III** 或 **IV–II**。使这些进行成为延伸的原因在于功能保持不变。**I–I6** 延伸主功能（**T1–T3**），**IV–II** 延伸下属功能（**S4–S2**）。

## Contrapuntal prolongation – passing chord｜对位性延伸：经过和弦
...
许多低音变换式延伸涉及低音的三度跳进，例如 **I–I6**（**T1–T3**）。正如在第二类对位中，从强拍到强拍的三度旋律音程会引致使用弱拍经过音一样，这两个和弦之间低音上的三度旋律音程会引致使用*经过和弦*。
...
经过和弦的低音会以级进填充这个三度。旋律中也常包含经过性运动。
...
一个功能通常由属于标准循环中位于它之前的那个功能的对位性和弦来延伸。T 由 D 延伸，D 由 S 延伸，S 由 T 延伸。
...
那么，用来延伸上述 **T1–T3** 进行的经过和弦，就是一个低音为 2 级音的属和弦（D 在 T 之前）（即 1 级与 3 级之间的经过音）：**D2**。

<iframe src="https://trinket.io/embed/music/026277e656" width="100%" height="350" frameborder="0" allowfullscreen></iframe>
...
常见的 **D2** 和弦有 **V6/4**、**V4/3** 和 **VII6**。因此，**I–I6** 延伸可以以这些和弦作为经过和弦。下面的 **T1 D2 T3** 进行用 **viiº6** 来延伸主功能。请聆听此例，然后试着把该进行改写为声部配置正确的 **I V6/4 I6** 进行。

<iframe src="https://trinket.io/embed/music/e02e8af4b3" width="100%" height="350" frameborder="0" allowfullscreen></iframe>
...
注意，虽然低音上的 2 级音可以支撑 **II** 和弦，但 **II** 是下属和弦，因此它*不*被用作延伸主功能的经过和弦。

## Contrapuntal prolongation – incomplete neighbor chord｜对位性延伸：不完全邻音和弦

In [second-species counterpoint](/posts/music/voice-leading/species-counterpoint/secondSpecies/), variety could come by using a *substitution* in place of a passing tone. This leap of a fourth followed by step in the opposite direction still outlines a third from downbeat to downbeat, but offers a break from too much stepwise motion in the counterpoint.

在 [second-species counterpoint｜第二类对位](/posts/music/voice-leading/species-counterpoint/secondSpecies/)中，可以用*替代音*取代经过音来获得变化。这种四度跳进后接反向级进，从强拍到强拍仍勾勒出一个三度，却能让对位从过多的级进运动中解脱出来。
...
在和声写作中，同样的效果由*不完全邻音和弦*获得。低音遵循与第二类对位相同的不完全邻音模式，而对位性和弦的功能与其经过和弦对应形式相同。因此，低音采用替代音模式时，产生的就不是 **T1 D2 T3** 的经过性运动，而是 **T1 D4 T3**。（用罗马数字表示，这一进行几乎总是 **I V4/2 I6**，如下例所示。）

<iframe src="https://trinket.io/embed/music/51a20e2c7a" width="100%" height="350" frameborder="0" allowfullscreen></iframe>

## Contrapuntal prolongation – complete neighbor chord｜对位性延伸：完全邻音和弦
...
正如第二类或第三类对位中的*邻音*可以用来装饰单个音并返回该音，*邻音和弦*利用低音上的邻音运动来延伸一个功能并返回原低音音高。邻音和弦的功能遵循与经过和弦或不完全邻音和弦相同的原则。以下是一些邻音和弦延伸的例子：
...
下面是一个严格键盘风格中的 **T1 D7 T1** 邻音延伸。**D7** 和弦的罗马数字与数字低音是什么？要把它变为 **T3 D4 T3**，最少需要作几处改动？

<iframe src="https://trinket.io/embed/music/e12906a55e" width="100%" height="350" frameborder="0" allowfullscreen></iframe>
...
正如第三类对位中有*双邻音*音型，和声也可以用两个和弦、通过在低音上使用双邻音音型来延伸。最常见的双邻音延伸是 **T1 D2 D7 T1**（通常写作 **I V4/3 V6/5 I**）。

<iframe src="https://trinket.io/embed/music/519c9f039d" width="100%" height="350" frameborder="0" allowfullscreen></iframe>

## Contrapuntal prolongation - divider and embellishing chords｜对位性延伸：分割和弦与装饰和弦
...
在第二类对位中，*音程分割*把强拍之间的大跳分成两个较小的跳进。同样地，*分割和弦*把低音变换式延伸中低音之间的一个大跳（或低音上单纯的八度跳进）分成两个较小的跳进。
...
分割和弦几乎总是延伸主功能，既可以用下属功能的分割和弦，也可以用属功能的分割和弦。最常见的分割和弦延伸有：

- **T1 D5 T1** (**I V(7) I**), where the bass ascends or descends an octave between **T1** chords.／**T1 D5 T1**（**I V(7) I**），其中低音在两个 **T1** 和弦之间上行或下行一个八度。
- **T1 S4 T1** (**I IV I**), where the bass ascends or descends an octave between **T1** chords.／**T1 S4 T1**（**I IV I**），其中低音在两个 **T1** 和弦之间上行或下行一个八度。
- **T1 S6 T3** (**I IV6 I6** or **I IV6 III**), dubbed the *champagne progression* by theorist Gene Biringer, because it is “the progression you pull out when you want to impress a date.”／**T1 S6 T3**（**I IV6 I6** 或 **I IV6 III**），被理论家 Gene Biringer 称为*香槟进行*，因为它是“你想在约会对象面前炫耀时祭出的进行”。
...
下面是一个香槟进行。它是哪一个版本（**I6** 还是 **III**）？要构成另一个版本，必须改变哪一处？这一进行“打破”了哪条默认的声部进行规则？（注意，由于规则之间的冲突，这一进行总会打破那条规则，并且其旋律中总是这些音级。）

<iframe src="https://trinket.io/embed/music/6a403a3d21" width="100%" height="350" frameborder="0" allowfullscreen></iframe>
...
在 **T1 D5 T1** 与 **T1 S4 T1** 的情形中，同样的和声进行可以在低音不改变音区的情况下发生。换言之，低音从 *do* 跳到 *sol* 或 *fa*，但返回原来的低音音高。此时中间那个和弦的低音并不是在分割一个大跳，而看起来像第三类对位中的*装饰音*。（在第三类对位中，*装饰音*通过跳到另一个协和音——通常相距三度或四度——再返回原音，来装饰另一个音。）因此，本来会是*分割和弦*的，就变成了*装饰和弦*。

Following is a **T1 D5 T1** *divider* prolongation. What single change can make it an *embellishing chord* prolongation?

下面是一个 **T1 D5 T1** 的*分割和弦*延伸。作哪一处改变就能使它成为*装饰和弦*延伸？

<iframe src="https://trinket.io/embed/music/a23ec68873" width="100%" height="350" frameborder="0" allowfullscreen></iframe>

## Subsidiary harmonic progressions｜次级的和声进行
...
最后一类延伸不是对位性的，而是涉及典型的 **T–(S)–D–T** 进行的弱化版本。当这样的进行未能产生恰当的终止式时——即它以 **D7–T1** 或 **D4–T3** 这类对位性和弦结束，或者用“欺骗性解决” **D5–T6**（**V–VI**）取代终止性的 **D5–T1**——该进行就称为*次级的和声进行*（这一术语来自 Edward Aldwell 与 Carl Schachter；Steven Laitz 把同一进行称为*嵌入式乐句模型*）。它之所以“次级”（或“嵌入式”），是因为它并不占据整个乐句，而是从属于（或嵌入于）一个更大的进行之中。
...
这些次级进行*总是延伸主功能*。在分析中的标注方式是：在起始的 T 之后接一条线：

> **T—————**

For instance, consider the following possible harmonic progression for a phrase:

例如，考虑下面这个乐句可能采用的和声进行：

<iframe src="https://trinket.io/embed/music/69e6b9acb9" width="100%" height="350" frameborder="0" allowfullscreen></iframe>
...
它第一次经历 **T–S–D–T** 循环、回到 T 时并未产生终止式。然而，它也不能被称为对位性延伸，因为它完美地遵循了正常的功能循环。因此，它是次级进行。

[![次级进行](/media/music/graphics/harmony/subsidiaryProgression.png)](/media/music/graphics/harmony/subsidiaryProgression.png)

## Plagal progressions｜变格进行
...
一般来说，**T** 用于对位性延伸 **S**，**S** 延伸 **D**，**D** 延伸 **T**。不过，也有一些常见模式是用 **S** 来延伸 **T**。
...
*香槟进行*（**I–IV6–I6** 或 **I–IV6–III**）是其中之一。另一个是 **S4** 分割和弦，以及与之相关的 **S4** 装饰和弦。所有这些上文都已介绍。
...
另一种常见模式是把 **IV**（**S**）用作 **I6**（**T**）的完全或不完全邻音。常见的进行包括 **I IV I6** 与 **I6 IV I6**。

<iframe src="https://trinket.io/embed/music/480c1acf9b" width="100%" height="350" frameborder="0" allowfullscreen></iframe>
...
同样常见的是 **T1** 的*数字低音变换式*延伸：**I–IV6/4–I**。**IV6/4** 可以视为一个 **S** 和弦，但更合适的做法往往是把该和弦中低音上方的六度和四度直接看作五度与三度的邻音。只需在罗马数字下方把这样的进行标为 **T———**。

<iframe src="https://trinket.io/embed/music/1e440d4768" width="100%" height="350" frameborder="0" allowfullscreen></iframe>

## Prolonging a progression｜延伸一个进行
...
有时，对位性和弦不是用来延伸单个功能，而是用来连接不同功能的和弦——换言之，用来延伸一个进行。
...
最常见的情形是低音线从 *do* 级进下行到 *sol*，这在小调中尤为常见。低音线 *do*–*te*–*le*–*sol* 配以 **T1 D7 S6 D5**（通常为 **i v6 iv6 V**——在这一称为“哀歌”的进行中，和弦性质很重要）。在这一进行中，**S6** 是通向终止性 **D5** 的功能性下属和弦。于是 **D7** 和弦只是一个连接 **T1** 与 **S6** 的经过和弦。要为它作标记，就在罗马数字分析下方 **T** 与 **S** 之间画一个箭头。

<iframe src="https://trinket.io/embed/music/3b6de8a510" width="100%" height="350" frameborder="0" allowfullscreen></iframe>
