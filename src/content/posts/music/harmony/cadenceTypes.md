...
sourceLink: https://openmusictheory.github.io/cadenceTypes.html
comment: true
draft: false
---
...
*终止式*是一个到达点，用于标示某个音乐单位的结束，例如乐句、主题、大型曲式段落或乐章。终止式同时是和声的、旋律的、节奏的和曲式的事件，但终止式通常是按照和声与旋律在标示该到达点时的不同方式来归类的。

## Authentic and half cadences｜正格终止与半终止

These unit-ending points of arrival are first grouped into *authentic cadences* and *half cadences*. An authentic cadence occurs when a formal unit ends with the progression **D5–T1** (**V(7)–I**, *sol* to *do* in the bass voice). If the melody accompanying this harmonic progression arrives on *do*, it is called a *perfect authentic cadence*; it the melody ends on *mi* or *me* (or more rarely *sol*), it is called an *imperfect authentic cadence*.

这些标示单位结束的到达点首先分为*正格终止*与*半终止*。当一个曲式单位以 **D5–T1**（**V(7)–I**，低音声部由 *sol* 到 *do*）的进行结束，即构成正格终止。若伴随这一和声进行的旋律到达 *do*，则称*完满正格终止*；若旋律结束在 *mi* 或 *me*（较少见的是 *sol*）上，则称*不完满正格终止*。
...
结束在 **V** 上而未进行到 **I** 的乐句称为*半终止*。这类终止的旋律通常包含 *re*，不过 *ti* 和 *sol* 也是可能的旋律到达音。该 **V** 几乎无一例外是三和弦而非七和弦，并且总是原位（**D5**）。
...
无论凭听觉还是借助乐谱，区分*完满正格终止*（PAC）、*不完满正格终止*（IAC）与*半终止*（HC），对曲式分析和模仿写作都至关重要。

## Simple, compound, and double cadences｜简单终止、复合终止与双重终止

Writers of Italian keyboard treatises like [Furno](http://faculty-web.at.northwestern.edu/music/gjerdingen/partimenti/collections/Furno/regoleP5.htm) and [Fenaroli](http://faculty-web.at.northwestern.edu/music/gjerdingen/partimenti/collections/Fenaroli/Regole/regoleP3.htm) (and, more recently, American historical theorists like [Robert O. Gjerdingen](http://faculty-web.at.northwestern.edu/music/gjerdingen/index.htm)) differentiate cadences according to the voice-leading found over the dominant harmony. These distinctions do not replace the above PAC/IAC/HC distinctions; rather they add another level of detail that is particularly helpful in model composition. These voice-leading distinctions provide three more cadence categories to complement PAC, IAC, and HC: the *simple cadence*, the *compound cadence*, and the *double cadence*.

意大利键盘论著的作者，如 [Furno](http://faculty-web.at.northwestern.edu/music/gjerdingen/partimenti/collections/Furno/regoleP5.htm) 和 [Fenaroli](http://faculty-web.at.northwestern.edu/music/gjerdingen/partimenti/collections/Fenaroli/Regole/regoleP3.htm)（以及晚近的美国历史理论家，如 [Robert O. Gjerdingen](http://faculty-web.at.northwestern.edu/music/gjerdingen/index.htm)），按照属和声之上的声部进行来区分终止式。这些区分并不取代上述 PAC／IAC／HC 的区分，而是增添了另一个层次的细节，对模仿写作尤其有帮助。这些声部进行的区分提供了三个额外的终止类别，与 PAC、IAC 和 HC 相互补充：*简单终止*、*复合终止*与*双重终止*。
...
*简单终止*指属和声功能由单个和弦——三和弦或七和弦——来体现。简单终止可用于 PAC、IAC 或 HC 的结构中，不过七和弦的版本通常只见于正格终止。
...
*复合终止*指终止属和弦的低音 *sol* 被重复，第二个 *sol* 常比第一个低一个八度。复合终止有三种具体形式。
...
复合终止的第一种类型是在属和声的 *sol* 低音之上使用 4–3 延留音——*do* 到 *ti*。在四声部织体中，另外两个声部保持低音上方五度和低音上方八度。*数字低音*标记通常是缩写形式 4–3，在四声部中它代表 8/5/4–8/5/3。4–3 延留音可以出现在 PAC、IAC 或 HC 的终止属和弦之上。

![4–3 compound cadence.](/media/music/graphics/cadences/Compound43.png)

![4–3 复合终止。](/media/music/graphics/cadences/Compound43.png)

![4–3 compound cadence with full thoroughbass figures.](/media/music/graphics/cadences/Compound43-full.png)

![带完整数字低音标记的 4–3 复合终止。](/media/music/graphics/cadences/Compound43-full.png)
...
复合终止的第二种类型在上述 4–3 延留音之外，增加一个 *mi/me* 到 *re* 的声部（6–5）。在四声部织体中，低音被重复。典型的数字低音标记是 6/4–5/3，由此产生了这一进行的通用名称——*终止四六和弦*。完整标记为 8/6/4–8/5/3。终止四六和弦可以出现在 PAC、IAC 或 HC 的终止属和弦之上。

![Cadential six-four.](/media/music/graphics/cadences/Cadential64.png)

![终止四六和弦。](/media/music/graphics/cadences/Cadential64.png)

![Cadential six-four with full thoroughbass figures.](/media/music/graphics/cadences/Cadential64-full.png)

![带完整数字低音标记的终止四六和弦。](/media/music/graphics/cadences/Cadential64-full.png)
...
复合终止的最后一种类型是终止四六和弦的特殊情形：第四个声部在第二个属和弦上引入七音，而不是在两个和弦上都简单地重复低音。这一复合终止类型需要四个声部，完整数字低音标记为 8/6/4–7/5/3。这一标记很少出现在半终止的属和弦之上，而主要保留给正格终止。

![Cadential six-four with seventh.](/media/music/graphics/cadences/Cadential64-7th.png)

![带七音的终止四六和弦。](/media/music/graphics/cadences/Cadential64-7th.png)
...
*双重终止*是在终止属和弦之上的四阶段模式，几乎只用于完满正格终止。尽管到莫扎特与海顿的时代它已不再常用，但它是早期*华丽风格*作曲家以及*古典*时期作曲与伴奏论著中的常见手法。属和弦之上的四阶段模式为 5/3–6/4–5/4–5/3。在四声部中，低音同样被重复，或者可对任何*五度和弦*（即不包括 6/4）加上七音。

![Double cadence.](/media/music/graphics/cadences/Double.png)

![双重终止。](/media/music/graphics/cadences/Double.png)

## Voice-leading in strict keyboard style｜严格键盘风格中的声部进行
...
在严格的旋律性键盘风格中，理想化乐句总应以完满正格终止（PAC）结束。旋律最后的 *do* 应尽可能级进进入，来自 *re* 或 *ti*，以来自 *re* 为佳。使用复合终止或双重终止时，采用上列谱例所示的上方各声部排列（当然要移到相应的调上）。
...
在*数字低音*风格中，最高声部可以结束在主三和弦的任一成员上。应尽可能使用上列谱例所给的各声部，但将其转位（例如把次中音声部移到最上方，使旋律与中音分别成为中音与次中音）。只需确保：若在最后的主和弦之前最高声部出现 *ti*，它须按其倾向解决到 *do*。
...
无论哪种情况，都要特别注意复合终止中的终止四六和弦形式。尽管它与低音构成协和三和弦，但第一个和弦的六度与四度都起延留音的作用，因此*必须级进下行解决*，并平滑地加以准备（通过共同音或级进）。无论哪一声部处于旋律、中音或次中音位置，这一点都成立。
