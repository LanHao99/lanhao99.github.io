...
description: "音符与休止符的时值体系，附点、延音线与连音线的用法。"
cover: /media/music/graphics/durations.png
...
sourceLink: https://openmusictheory.github.io/rhythmicValues.html
comment: true
draft: false
---
...
节奏指长短时值在时间上的组合。时值用空心或实心符头来记写。空心符头可以带符干，也可以不带符干；实心符头总是带符干。实心符头的符干上可以添加符尾；每一条符尾使时值缩短一半。

<img src="/media/music/graphics/durations.png" alt="各种时值的音符" width="60%" />

## Rests｜休止符

Rests represent silence in musical notation. For each durational symbol there exists a corresponding rest.

休止符在音乐记谱中表示静默。每一个时值符号都有一个与之对应的休止符。

<img src="/media/music/graphics/rests.png" alt="各种时值的休止符" width="60%" />

## Dots and ties｜附点与延音线
...
附点与延音线可使基本时值得到延长。附点写在音或休止符之后，将其时值增加一半。例如，若一个四分音符的时值相当于两个八分音符，那么一个附点四分音符就相当于*三个*八分音符。一般来说，不带附点的音符二等分为两个音符；带附点的音符三等分为三个音符。因此，不带附点的音符通常用于表示单拍子中的单位拍层级，而带附点的音符用于表示复拍子中的单位拍。
...
一个时值可以添加多个附点。后续的附点增加前一个附点时值的一半。例如，带两个附点的四分音符，其时值相当于一个四分音符、一个八分音符与一个十六分音符之和。

[<img src="/media/music/graphics/dots.png" alt="附点的用法" width="50%" />](/media/music/graphics/dots.png)
...
*延音线*通过连接两个相邻的相同音高来延长时值。延音线用于把某个音延续到超过一小节的长度，或使小节中某种特定的节奏分组更加清晰。
...
在下面的例子中，第一个音的时值长于一小节，因此用延音线把占满整小节的附点二分音符与后一小节的第一拍连接起来表示。

[<img src="/media/music/graphics/ties.png" alt="延音线的用法" width="50%" />](/media/music/graphics/ties.png)
