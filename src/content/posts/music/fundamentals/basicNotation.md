---
title: "基本记谱法"
published: 2022-01-02
description: "五线谱、谱号、加线与变音记号等西方音乐记谱的基本要素。"
cover: /media/music/graphics/noteillustration.png
coverInContent: false
pinned: false
tags: [乐理, 记谱法]
category: [音乐理论, 音乐基础]
author: Open Music Theory
licenseName: CC BY-SA 4.0
licenseUrl: https://creativecommons.org/licenses/by-sa/4.0/
sourceLink: https://openmusictheory.github.io/basicNotation.html
comment: true
draft: false
---

*Basic notation*

## Notes｜音符

When written on a staff, a note indicates a pitch and rhythmic value. The notation consists of a *notehead* (either empty or filled in), and optionally can include a *stem*, *beam*, *dot*, or *flag*.

写在五线谱上时，一个音符同时指示音高与时值。其记谱形态包括一个*符头*（空心或实心），并可选择附带*符干*、*符杠*、*附点*或*符尾*。

<img src="/media/music/graphics/noteillustration.png" alt="音符示意图" width="50%" />

## Staff｜五线谱

Notes can’t convey their pitch information without being placed on a staff. A staff consists of five horizontal lines, evenly spaced. The plural of staff is *staves*.

若不置于五线谱上，音符便无法传达其音高信息。五线谱由五条水平线组成，线间距离均匀。staff 的复数形式是 *staves*。

## Clefs｜谱号

Notes *still* can’t convey their pitch information if the staff doesn’t include a clef. A clef indicates which pitches are assigned to the lines and spaces on a staff. The two most commonly used clefs are the *treble* and *bass* clef; others that you’ll see relatively frequently are *alto* and *tenor* clef.

如果五线谱上没有谱号，音符*仍然*无法传达其音高信息。谱号指示五线谱上的线与间分别对应哪些音高。最常用的两种谱号是*高音谱号*与*低音谱号*；相对常见的还有*中音谱号*与*次中音谱号*。

Here is the pitch C4 placed on the treble, bass, alto, and tenor clefs.

下例是音高 C4 分别置于高音、低音、中音与次中音谱号上的情形。

<img src="/media/music/graphics/clefs.png" alt="各种谱号" width="50%" height="50%" />

## Grand staff｜大谱表

The grand staff consists of two staves, one that uses a treble clef, and one that uses a bass clef. The staves are connected by a curly brace. Grand staves are used frequently for notating piano music and other polyphonic instruments.

大谱表由两行五线谱组成，一行使用高音谱号，另一行使用低音谱号。两行谱表由花括号连接。大谱表常用于记写钢琴音乐及其他复调乐器音乐。

## Ledger lines｜加线

When the music’s range exceeds what can be written on the staff, extra lines are drawn so that we can still clearly read the pitch. These extra lines are called *ledger lines.* In the example below, From Haydn’s Piano Sonata in G (Hob. XVI: 39), Ab5 occurs just above the treble staff in the right hand, and G3 and B3 occur just below the treble staff in the left hand.

当音乐的音域超出五线谱所能记录的范围时，会额外画出短线，以便我们仍能清楚地读出音高。这些额外的线称为*加线*。下例选自海顿《G 大调钢琴奏鸣曲》（Hob. XVI: 39），右手声部的 Ab5 位于高音谱表上方紧邻处，左手声部的 G3 与 B3 位于高音谱表下方紧邻处。

<img src="/media/music/graphics/ledgerLines.png" alt="加线" width="80%" height="80%" />

## Accidentals｜变音记号

Accidentals are used to indicate when a pitch has been raised or lowered. They are written to the *left* of the pitch.

变音记号用于指示音高被升高或降低。它们写在音符的*左侧*。

- When you lower one of the white notes of the piano by a semitone, you add a flat.／将钢琴上的某个白键音降低半音时，添加降记号。
- When you raise one of the white notes of the piano by a semitone, you add a sharp.／将钢琴上的某个白键音升高半音时，添加升记号。
- When you raise a note that is already flat by a semitone, you add a natural.／将已被降低的音再升高半音时，添加还原记号。
- When you lower a note that is already flat by a semitone, you add a double flat.／将已被降低的音再降低半音时，添加重降记号。
- When you raise a note that is already sharp by a semitone, you add a double sharp.／将已被升高的音再升高半音时，添加重升记号。

The example below shows the symbols for flat, natural, sharp, double sharp, and double flat, respectively.

下例依次展示降记号、还原记号、升记号、重升记号与重降记号的符号。

<img src="/media/music/graphics/accidentals.png" alt="变音记号" width="50%" height="50%" />
