...
sourceLink: https://openmusictheory.github.io/intervals.html
comment: true
draft: false
---
*Intervals and dyads*
...
*二音音组*是同时发响的一对音高（换言之，即双音和弦）。由于二音音组是由两音之间的音程来界定的，因此二音音组常被径直称作音程。
...
因此，*音程*这一术语通常既指音阶上两音之间的距离，也指两音之间相隔该距离的二音音组。

## Chromatic intervals｜半音音程
...
计量音程最简单的方法——尤其是在键盘上——是数出两音之间包含多少个半音。例如，要确定 C4 与 E4 之间的半音音程，就从 C4 出发，沿半音阶上行至 E4，并逐一数出途中的级数：C#4、D4、D#4、E4。E4 比 C4 高四个半音。半音音程用小写 **i** 加表示半音数的阿拉伯数字来标记。C4–E4 为四个半音，即 **i4**。

![从 C4 到 E4 的半音级数](/media/music/graphics/intervals/C4E4.png)
...
在调性音乐中，我们更常关注的是自然（大调或小调）音阶上的级数。这一点略有蹊跷——并非因为它难，而是因为它有悖直觉。遗憾的是，这套体系太过古老、也太过根深蒂固，如今已无从更改！不过一旦越过最初的不适感，全音阶音程还是可以掌握的。
...
辨认全音阶音程时，先只看*音名字母*。也就是说，暂时把 C、升 C 和降 C 都当作 *C*。接着，数出所论两音之间的级数（不同字母的个数），*计数时把两端的音都算进去*。这样得到的就是*一般音程*。
...
例如，从 C4 到 E4，把 C 与 E 都计入，其间共有三个全音阶级数（三个音名字母）：C、D、E。因此，C4–E4 的一般音程是*三度*。任何 C 到任何 E 都是如此：C#4 到 E4、Cb4 到 E#4 等等。它们都是全音阶三度。

![三种一般三度](/media/music/graphics/intervals/thirds.png)
...
一般音程所能提供的往往还不够精确。这种精确性来自音程的*性质*。把*性质*与一般音程的名称结合起来，便得到*具体音程*。
...
要得到音程的性质，须同时求出其一般音程与半音音程，然后查下表得出具体音程。

|  | unis.／纯一度 | 2nd／二度 | 3rd／三度 | 4th／四度 | 5th／五度 | 6th／六度 | 7th／七度 | oct.／八度 |
| ---: | :---: | :---: | :---: | :---: | :---: | :---: | :---: | :---: |
| i0 | P1 | d2 |  |  |  |  |  |  |
| i1 | A1 | m2 |  |  |  |  |  |  |
| i2 |  | M2 | d3 |  |  |  |  |  |
...
例如，C4–E4 是一般三度，其半音音程为 i4。包含四个半音的三度是*大三度*（M3）。请注意，要确定具体音程，一般音程与半音音程二者缺一不可，因为对每一个一般音程、每一个半音音程而言，对应的具体全音阶音程都不止一个。
...
请注意，有些一般音程可以是增、纯或减，另一些音程则可以是增、大、小或减。没有任何一种一般音程既能是大／小、又能是纯；若它能是大或小，就不能是纯；若它能是纯，就不能是大或小。音程的增形式总是比大或纯宽一个半音；减形式总是比小或纯窄一个半音。
...
唱名同样有助于确定具体音程。只要明确了哪个唱名是下方音、哪个是上方音，那么无论什么调，每一对唱名之间的音程都相同。记住各唱名对之间的音程，有助于加快对音乐中出现的二音音组的分析。例如，知道 *do*–*mi*、*fa*–*la*、*sol*–*ti* 永远是大三度，并知道 *re*–*fa*、*mi*–*sol*、*la*–*do*、*ti*–*re* 永远是小三度，就能更快地分析大调中的二音音组。
...
上文讨论的从同度到八度的音程称为*单音程*。任何大于八度的音程都视为*复音程*。以 C4 到 E5 的音程为例，其一般音程是十度。然而在几乎所有音乐情境中，它的功能都与 C4 到 E4 相同。因此，十度 C4–E5 也被称作*复三度*。复音程的性质与相应的单音程相同。若 C4–E4 是大三度，那么 C4–E5 就是大十度。

![单音程与复音程的大三度](/media/music/graphics/intervals/compound.png)
...
除 C4–E4 与 C4–E5 之外，E4–C5 也具有相似的音响与音乐功能。事实上，任何保持相同两个音级而改变音区的二音音组，都会有相似的音响与功能。然而，E4–C5 的最低音是 E 而非 C，这一事实意味着它具有不同的一般音程：E4–C5 是六度，而非三度。正因这一差异，它在某些情境中也会承担不同的音乐功能。不过，这一层关系终究无法回避。
...
由相同两个音级构成、但上下位置互换的二音音组，彼此称为对方的*转位*，因为这些音级被*倒置*了。同样，由这些倒置的二音音组所标定的音程，彼此也称为对方的*转位*。
...
再以 C4–E4（大三度）与 E4–C5（小六度）为例。这两个二音音组拥有相同的两个音级，但前者下方为 C、上方为 E，后者下方为 E、上方为 C。因此，它们互为转位。

![转位关系：大三度与小六度](/media/music/graphics/intervals/inversion.png)

Three relationships exhibited by these two dyads hold for all interval inversions.

这两个二音音组所体现的三条关系，适用于所有音程转位。
...
第一，半音音程之和为 12。（C4–E4 = i4；E4–C5 = i8；i4 + i8 = i12）这是因为两个音程相加正好是一个八度（在 E4 处有重叠）。
...
第二，*两个一般音程的数值之和为九*（三度加六度，即 3 + 6）。这是因为两个音程相加为一个八度（8），而相加时有一个音被数了两次。（回想一下全音阶音程那种有悖直觉的数法：数目包含起止两音；而把互为转位的音程相加时，总有一个音会被数两次——在此例中即 E4。）
...
最后，大音程转位为小音程，反之亦然。这在音程转位中始终成立。同样，增音程的转位总是减音程，反之亦然。纯音程的转位总是纯音程。
...
音程转位眼下或许显得费解而玄奥，但它将是研习声部进行与和声时极为重要的概念。

## Methods for learning intervals｜学习音程的方法

There are several methods for learning intervals. Choose your favorite:

学习音程有若干种方法。可择你偏爱者：

- [The white-key method｜白键法](https://openmusictheory.github.io/whiteKeyMethod.html)

## Melodic and harmonic intervals｜旋律音程与和声音程
...
音程类型中最后一点需要留意的区分，是*旋律*音程与*和声*音程之别。这一区分很简单。若二音音组中的两个音高同时发响（即双音和弦），则它们之间的音程是*和声音程*。若所论的两个音高一前一后发响（如旋律中那样），则它们之间的音程是*旋律音程*。这一区分在声部进行中很重要，因为在和声语境与旋律语境中，被偏好或被禁用的音程各不相同。这一差异对听辨也很重要，因为听辨性质相同的旋律音程与和声音程需要不同的技巧。

## Consonance and dissonance｜协和与不协和
...
音程依据其音响（听起来有多稳定、悦耳或刺耳）、演唱的难易程度，以及在一段音乐中（开头、中间、结尾；处在某些其他音程之间等等）最恰当的功能，被归类为*协和*或*不协和*。旋律与和声适用不同的标准。以下类别对你进行严格声部进行写作至关重要，同时也将是自由创作与编配工作的有益指南。

**Melodic consonance and dissonance｜旋律的协和与不协和**
...
以下*旋律*音程是*协和的*，在严格声部进行中既可用于相邻音高之间，也可用作单一方向级进进行的边界：

- All perfect intervals (P4, P5, P8)／所有纯音程（P4、P5、P8）
- All diatonic steps (M2, m2)／所有全音阶级进（M2、m2）
- Major and minor thirds／大三度与小三度
- Major and minor sixths／大六度与小六度
...
所有其他*旋律*音程都是*不协和的*，在相邻音高之间以及作为单一方向级进进行的边界时都必须避免，包括：

- All augmented and diminished intervals (including those that are enharmonically equivalent to consonant intervals, such as A2 and A1)／所有增音程与减音程（包括与协和音程等音等值的那些，如 A2 与 A1）
- All sevenths／所有七度

**Harmonic consonance and dissonance｜和声的协和与不协和**
...
以下*和声*音程是*不完全协和音程*，在严格声部进行中可相对自由地使用（开头与结尾除外）：
...
以下*和声*音程是*完全协和音程*，在严格声部进行中必须谨慎使用于有限的情境：

- All perfect intervals *except the perfect fourth* (P1, P5, P8)／除*纯四度*以外的所有纯音程（P1、P5、P8）

All other *harmonic* intervals are *dissonant*, and must be employed in very specific ways in strict voice-leading, including:

所有其他*和声*音程都是*不协和的*，在严格声部进行中必须以非常特定的方式运用，包括：
