---
title: "Key signatures｜调号"
published: 2022-01-09
description: "五度圈、各调号在四种谱号中的写法与识别方法。"
cover: /media/music/graphics/melodyWithoutKS.png
coverInContent: false
pinned: false
tags: [乐理, 调号]
category: [音乐理论, 音乐基础]
author: Open Music Theory
licenseName: CC BY-SA 4.0
licenseUrl: https://creativecommons.org/licenses/by-sa/4.0/
sourceLink: https://openmusictheory.github.io/keySignatures.html
comment: true
draft: false
---

> **来源**：本文选自 [Open Music Theory](https://openmusictheory.github.io/keySignatures.html)，由 Hybrid Pedagogy Publishing 出版，采用 [CC BY-SA 4.0](https://creativecommons.org/licenses/by-sa/4.0/) 许可。
>
> **中文名**：调号　|　**归档说明**：中英对照，每段英文原文后附中文译文，英文原文未作删减；图片与音频已本地化，站内链接已改为本站链接。

When you’re writing in a single key for an extended period of time, it gets tedious to write out the accidentals over and over again.

当你长时间在同一个调上写作时，一遍又一遍地写出变音记号会变得十分累赘。

Here is a simple melody in D major, without a key signature.

下面是一段 D 大调的简单旋律，未使用调号。

[![无调号的旋律](/media/music/graphics/melodyWithoutKS.png)](/media/music/graphics/melodyWithoutKS.png)

To avoid this, composers used *key signatures* at the beginning of each staff to remind performers of which pitch classes should have flats or sharps.

为避免这种累赘，作曲家在每行谱表的开头使用*调号*，以提示演奏者哪些音级应当带降号或升号。

Here is the same melody, with the key signature at the beginning of the staff to remind the performer that F and C should be sharp.

下面是同一段旋律，谱表开头带有调号，提示演奏者 F 与 C 应当升高。

[![带调号的旋律](/media/music/graphics/melodyWithKS.png)](/media/music/graphics/melodyWithKS.png)

## The circle of fifths｜五度圈

The circle of fifths is an illustration that has been used in music theory pedagogy for hundreds of years. It conveniently summarizes the key signature needed for any key with up to seven flats or sharps.

五度圈是一幅已在音乐理论教学中使用数百年的图示。它便捷地概括了升降号不超过七个的任何调所需的调号。

[<img src="/media/music/graphics/circleOfFifths.png" alt="五度圈（大调）" width="65%" />](/media/music/graphics/circleOfFifths.png)

But *which* notes are flat or sharp in a key? To properly use the circle of fifths to figure out a key signature, you’ll need to also remember this mnemonic device, which tells you the order of flats and sharps:

但一个调中*哪些*音是降的或升的？要以五度圈正确地推出调号，你还需要记住下面这个助记口诀，它告诉你降号与升号的排列顺序：

**F**ather **C**harles **G**oes **D**own **A**nd **E**nds **B**attle.

**F**ather **C**harles **G**oes **D**own **A**nd **E**nds **B**attle.（依次首字母对应 F、C、G、D、A、E、B）

For sharp keys (clockwise on the circle of fifths), read the mnemonic device forward. For example, the circle of fifths tells us that there are 3 sharps in the key of A major. Which three notes are sharp? The first three notes in the mnemonic device: F(ather), C(harles), and G(oes).

对升号调（在五度圈上顺时针方向），顺读该助记口诀。例如，五度圈告诉我们 A 大调有三个升号。是哪三个音升高？即口诀中的前三个音：F(ather)、C(harles) 与 G(oes)。

For flat keys (counter-clockwise on the circle of fifths), read the mnemonic device backwards. For example, the circle of fifths tells us that the key of A-flat major has four flats. Which flats? Reading backwards: B(attle), E(nds), A(nd), D(own).

对降号调（在五度圈上逆时针方向），倒读该助记口诀。例如，五度圈告诉我们降 A 大调有四个降号。是哪四个降号？倒读即：B(attle)、E(nds)、A(nd)、D(own)。

## Minor key signatures｜小调调号

Of course, minor keys can use key signatures, too. In fact, for each major key signature, there is a corresponding minor key that shares its signature. Major and minor keys that share the same key signature are called *relative* keys. For example, both C major and A minor have zero sharps or flats. A minor is considered the *relative minor* of C major; likewise, C major is considered the *relative major* of A minor. Compare the minor key circle of fifths below with the major key circle of fifths above, and you’ll see the remaining relative key pairs.

当然，小调也可以使用调号。事实上，每一个大调调号都对应一个共用该调号的小调。共用同一调号的大调与小调称为*关系*调。例如，C 大调与 a 小调都没有升降号。a 小调被视为 C 大调的*关系小调*；同样，C 大调被视为 a 小调的*关系大调*。把下面小调的五度圈与上面大调的五度圈对照来看，你就能看到其余各对关系调。

[<img src="/media/music/graphics/circleOfFifths-minor.png" alt="五度圈（小调）" width="65%" />](/media/music/graphics/circleOfFifths-minor.png)

## Writing key signatures｜调号的写法

Below is a reference that shows how all of the key signatures should be written on treble, alto, tenor, and bass clefs.

下面这份参考资料展示了所有调号在高音、中音、次中音与低音谱号上应如何书写。

[![高音谱号上的升号调](/media/music/graphics/sharpsTreble.png)](/media/music/graphics/sharpsTreble.png)
[![中音谱号上的升号调](/media/music/graphics/sharpsAlto.png)](/media/music/graphics/sharpsAlto.png)
[![次中音谱号上的升号调](/media/music/graphics/sharpsTenor.png)](/media/music/graphics/sharpsTenor.png)
[![低音谱号上的升号调](/media/music/graphics/sharpsBass.png)](/media/music/graphics/sharpsBass.png)
[![高音谱号上的降号调](/media/music/graphics/flatsTreble.png)](/media/music/graphics/flatsTreble.png)
[![中音谱号上的降号调](/media/music/graphics/flatsAlto.png)](/media/music/graphics/flatsAlto.png)
[![次中音谱号上的降号调](/media/music/graphics/flatsTenor.png)](/media/music/graphics/flatsTenor.png)
[![低音谱号上的降号调](/media/music/graphics/flatsBass.png)](/media/music/graphics/flatsBass.png)
