...
sourceLink: https://openmusictheory.github.io/pitches.html
comment: true
draft: false
---
...
键盘非常有助于培养对乐理的视觉、听觉与触觉理解。在下图中，*音级*的字母名称被标注在键盘上。

![键盘布局（小图）](/media/music/graphics/keyboardlayout-small.png)

## Enharmonic equivalence｜等音等值
...
请注意，有些键有两个名称。当两个音级在键盘上共用同一个键时，称它们具有*等音等值*关系。理论上，每个键都可以有若干名称（例如 C 音也可视为 D♭♭），但通常只需知道两种等音写法即可。

## Octave Designation｜八度标记
...
要精确指明某一个音高时，我们还需要知道*音区*。事实上，如果你手里只有「升 C」或「降 B」，那你拥有的并不是一个*音高*，而是一个*音级*。音级加上音区，才共同指认出一个具体的音高。
...
我们将遵循国际标准化组织（ISO）的音区标记体系。在该体系中，中央 C（低音谱表上方第一条加线，或高音谱表下方第一条加线）为 C4。比中央 C 高一个八度的是 C5，低一个八度的是 C3。
...
这套体系略显棘手之处在于：八度从 C 开始，到 B 结束。因此从中央 C 开始的上行音阶包含以下音高标记：

![C4 到 C5](/media/music/graphics/C4toC5.png)

And a descending scale from middle C contains the following pitch designations:

而从中央 C 开始的下行音阶包含以下音高标记：

![C4 到 C3](/media/music/graphics/C4toC3.png)

Pitches on the alto staff are as follows:

中音谱表上的音高如下：

![F3 到 G4（中音谱号）](/media/music/graphics/F3toG4-alto.png)

Pitches on the tenor staff are as follows:

次中音谱表上的音高如下：

[<img src="/media/music/graphics/pitchesTenor.png" alt="次中音谱表上的音高" width="70%" />](/media/music/graphics/pitchesTenor.png)
...
任何变音记号都沿用同一自然音名的八度标记。因此，比 C4 低半音的是 C-flat4（尽管它听起来与 B3 相同），比 C4 高半音的是 C-sharp4。
...
请注意，完整的标记同时包含音级名称（一个字母名加上可选的升号或降号）与音区（标明该音所在八度的 ISO 编号）。除非两者齐备，否则你并没有得到一个具体音高的完整标记。
