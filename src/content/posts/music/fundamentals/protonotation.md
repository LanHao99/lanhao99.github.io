---
title: "Protonotation｜原生记谱法"
published: 2022-01-04
description: "脱离五线谱的节奏记谱训练法，用简单符号记录节拍、分组与音高轮廓。"
cover: /media/music/graphics/protonotation/demoMel.png
coverInContent: false
pinned: false
tags: [乐理, 节奏]
category: [音乐理论, 音乐基础]
author: Open Music Theory
licenseName: CC BY-SA 4.0
licenseUrl: https://creativecommons.org/licenses/by-sa/4.0/
sourceLink: https://openmusictheory.github.io/protonotation.html
comment: true
draft: false
---

> **来源**：本文选自 [Open Music Theory](https://openmusictheory.github.io/protonotation.html)，由 Hybrid Pedagogy Publishing 出版，采用 [CC BY-SA 4.0](https://creativecommons.org/licenses/by-sa/4.0/) 许可。
>
> **中文名**：原生记谱法　|　**归档说明**：中英对照，每段英文原文后附中文译文，英文原文未作删减；图片与音频已本地化，站内链接已改为本站链接。

*Protonotation* is a system of musical notation stripped of complicating elements, and focusing only on basic elements of meter, rhythm, and scale degree. (This system is drawn from Gary Karpinski’s *Manual for Ear Training and Sight Singing*.) Following is an example melody in both standard notation and protonotation (click images to view full size).

*原生记谱法*是一种剥离了繁复要素的音乐记谱体系，只着眼于节拍、节奏与音级这些基本要素。（该体系取自 Gary Karpinski 的《Manual for Ear Training and Sight Singing》。）下面是一段示例旋律，分别以标准记谱法与原生记谱法呈现（点击图片可查看大图）。

[![示例旋律的原生记谱](/media/music/graphics/protonotation/demoMel.png)](/media/music/graphics/protonotation/demoMel.png)

[![示例旋律的五线谱](/media/music/graphics/protonotation/demoMelStaff.png)](/media/music/graphics/protonotation/demoMelStaff.png)

Note that protonotation does not contain information about *tonic*, nor the *duration of the beat* (nor, by extension, the bottom number of the time signatures). It only represents the pitch and rhythm elements that can be heard by someone without absolute pitch: meter, rhythm, and scale degree. (Mode—major or minor—can be inferred from the scale degrees.) Since it only represents what can be heard, without any additional notational factors, it is a helpful system for practicing rhythmic dictation, melodic dictation, and sight-singing.

请注意，原生记谱法不包含*主音*的信息，也不包含*单位拍的时值*（由此也不包含拍号的下方数字）。它只呈现没有绝对音感的人也能听出的音高与节奏要素：节拍、节奏与音级。（调式——大调或小调——可由音级推断出来。）由于它只呈现能被听到的内容，不带任何额外的记谱因素，因此是练习节奏听写、旋律听写与视唱的有益体系。

## Elements of protonotation｜原生记谱法的要素

Following are blank protonotation grids for duple, triple, and quadruple meters.

以下是二拍子、三拍子与四拍子的空白原生记谱格。

*Duple meter:*

*二拍子：*

![二拍子](/media/music/graphics/protonotation/duple.png)

*Triple meter:*

*三拍子：*

![三拍子](/media/music/graphics/protonotation/triple.png)

*Quadruple meter:*

*四拍子：*

![四拍子](/media/music/graphics/protonotation/quadruple.png)

In duple and triple meter, downbeats are represented by longer vertical lines, and weak beats are represented by shorter vertical lines. In quadruple meter, the third beat of each bar is of medium strength, so it is represented by a medium-length line.

在二拍子与三拍子中，强拍（第一拍）用较长的竖线表示，弱拍用较短的竖线表示。在四拍子中，每小节的第三拍强度居中，因此用长度居中的竖线表示。

Notes are notated by using horizontal lines for rhythmic duration and moveable-*do* solfège syllables for scale degree. Arrows are used to denote the direction of any melodic leaps. (Arrows are not necessary for stepwise progressions.)

音符的记写方式是：用横线表示时值，用可动*do*唱名表示音级。箭头用于标示任何旋律跳进的方向。（级进进行无需箭头。）

Rests are represented by the lack of horizonal line in a given beat or part of a beat. When using protonotation for transcription or dictation, however, it can be helpful to use an **X** instead of a blank, so you can distinguish a rest you are sure about from a part of the music you have left blank because you have not yet determined what is going on at that moment.

休止由某一拍或某拍的一部分中没有横线来表示。不过，在用原生记谱法做转写或听写时，用一个 **X** 代替空白会有所帮助，这样你就能把确定无疑的休止与因尚未判断出当时发生了什么而留白的部分区分开来。

## Converting protonotation to staff notation｜把原生记谱法转换为五线谱

Protonotation is not enough to produce staff notation. However, if you know 1) the clef, 2) the tonic pitch, and 3) either the beat duration or bottom number of the time signature, you can convert it to staff notation easily.

仅凭原生记谱法不足以生成五线谱。不过，如果你知道 1) 谱号，2) 主音音高，以及 3) 单位拍时值或拍号的下方数字，就能轻易地把它转换成五线谱。

First, draw the clef provided (or choose an appropriate one based on your perception of the register of the melody) and determine the key signature from the tonic provided and the mode you heard. (Look through your melody for *mi* v. *me*.)

首先，画出所给的谱号（或依据你对旋律音区的感知选择一个合适的谱号），并根据所给的主音与你听到的调式确定调号。（在你的旋律中查找 *mi* 与 *me* 的差别。）

Then, determine the time signature from the beat value/bottom number provided and from the meter refelected in your protonotation. (Review [the relationship of meter to time signature](/posts/music/fundamentals/meter/), if necessary.) If no bottom number is provided, choose a convenient one (**4** for simple meters and **8** for compound meters are the most typical).

接着，根据所给的单位拍时值／下方数字，以及你的原生记谱中所反映的节拍，确定拍号。（如有必要，请复习[节拍与拍号的关系](/posts/music/fundamentals/meter/)。）若未给出下方数字，就选择一个方便的数值（**4** 用于单拍子、**8** 用于复拍子是最典型的）。

Next, each of the long protonotation lines become barlines in staff notation.

然后，原生记谱中的每一条长线在五线谱中都变成小节线。

Finally insert the notes into each bar. The register, solfège syllable, and tonic will determine the pitch class. The rhythmic value will be determined by the duration of the note in beats and what the time signature implies about the duration of the beat, For example, a two-beat note in 4/4 is a half note (2 x quarter-note beat); in 2/2 is a whole note (2 x half-note beat); in 9/8 is a dotted half note (2 x dotted-quarter-note beat); and in 6/4 is a dotted whole note (2 x dotted-half-note beat).

最后把音符填入每个小节。音区、唱名与主音共同决定音级。时值则由音符所占的拍数以及拍号所暗示的单位拍时值决定。例如，一个两拍的音在 4/4 中是二分音符（2 × 四分音符拍）；在 2/2 中是全音符（2 × 二分音符拍）；在 9/8 中是附点二分音符（2 × 附点四分音符拍）；在 6/4 中是附点全音符（2 × 附点二分音符拍）。

Following are additional examples.

以下为补充示例。

**Example 1.**

**例 1.**

[![二拍子旋律](/media/music/graphics/protonotation/dupleMel.png)](/media/music/graphics/protonotation/dupleMel.png)

Bass clef, tonic: G, bottom number: 8.

低音谱号，主音：G，下方数字：8。

[![二拍子，G 主音](/media/music/graphics/protonotation/duple-G.png)](/media/music/graphics/protonotation/duple-G.png)

Alto clef, tonic: B-flat, bottom number: 4.

中音谱号，主音：降 B，下方数字：4。

[![二拍子，降 B 主音](/media/music/graphics/protonotation/duple-Bb.png)](/media/music/graphics/protonotation/duple-Bb.png)

**Example 2.**

**例 2.**

[![四拍子旋律](/media/music/graphics/protonotation/quadrupleMel.png)](/media/music/graphics/protonotation/quadrupleMel.png)

Treble clef, tonic: E-flat, bottom number: 4.

高音谱号，主音：降 E，下方数字：4。

[![四拍子，降 E 主音](/media/music/graphics/protonotation/quadruple-Es.png)](/media/music/graphics/protonotation/quadruple-Es.png)

Tenor clef, tonic: C-sharp, bottom number: 8.

次中音谱号，主音：升 C，下方数字：8。

[![四拍子，升 C 主音](/media/music/graphics/protonotation/quadruple-Cis.png)](/media/music/graphics/protonotation/quadruple-Cis.png)

Bass clef, tonic: F, bottom number: 1.

低音谱号，主音：F，下方数字：1。

[![四拍子，F 主音](/media/music/graphics/protonotation/quadruple-F.png)](/media/music/graphics/protonotation/quadruple-F.png)
