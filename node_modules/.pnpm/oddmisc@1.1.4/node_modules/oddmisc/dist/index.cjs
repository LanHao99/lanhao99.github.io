...
`;t("page",a)}}}:{name:"oddmisc-umami-integration",hooks:{}}}function K(r){let t=new URL(r),e=t.pathname.split("/"),a=e.indexOf("share");if(a===-1||a===e.length-1)throw new Error("\u65E0\u6548\u7684\u5206\u4EAB URL\uFF1A\u672A\u627E\u5230 share \u8DEF\u5F84");let s=e[a+1];if(!s)throw new Error("\u65E0\u6548\u7684\u5206\u4EAB URL\uFF1A\u7F3A\u5C11\u5206\u4EAB ID");let n=e.slice(0,a).join("/")+"/api";return{apiBase:`${t.protocol}//${t.host}${n}`,shareId:s}}var y=class{constructor(t,e){this.cache=new Map;this.storageKey=t,this.ttl=e,this.loadFromStorage()}loadFromStorage(){try{let t=localStorage.getItem(this.storageKey);if(t){let e=JSON.parse(t),a=Date.now();for(let[s,i]of Object.entries(e))a-i.timestamp<this.ttl&&this.cache.set(s,i)}}catch{}}saveToStorage(){try{let t={};this.cache.forEach((e,a)=>{t[a]=e}),localStorage.setItem(this.storageKey,JSON.stringify(t))}catch{}}get(t){let e=this.cache.get(t);return e&&Date.now()-e.timestamp<this.ttl?e.value:null}set(t,e){this.cache.set(t,{value:e,timestamp:Date.now()}),this.saveToStorage()}clear(){this.cache.clear();try{localStorage.removeItem(this.storageKey)}catch{}}},C=class{constructor(t){this.shareData=null;this.sharePromise=null;let{apiBase:e,shareId:a}=K(t.shareUrl);this.apiBase=e,this.shareId=a,this.cache=new y(`umami-runtime-${a}`,36e5)}async getShareData(){return this.shareData?this.shareData:this.sharePromise?this.sharePromise:(this.sharePromise=(async()=>{let t=await fetch(`${this.apiBase}/share/${this.shareId}`);if(!t.ok)throw this.sharePromise=null,new Error(`\u83B7\u53D6\u5206\u4EAB\u4FE1\u606F\u5931\u8D25: ${t.status}`);let e=await t.json();return this.shareData=e,e})(),this.sharePromise)}async getStats(t){let e=t?`stats-${t}`:"stats-site",a=this.cache.get(e);if(a)return{...a,_fromCache:!0};let{websiteId:s,token:i}=await this.getShareData(),n=new URLSearchParams({startAt:"0",endAt:Date.now().toString()});t&&n.set("path",`eq.${t}`);let c=await fetch(`${this.apiBase}/websites/${s}/stats?${n.toString()}`,{headers:{"x-umami-share-token":i}});if(!c.ok)throw new Error(`\u83B7\u53D6\u7EDF\u8BA1\u5931\u8D25: ${c.status}`);let o=await c.json(),g={pageviews:o.pageviews?.value??o.pageviews??0,visitors:o.visitors?.value??o.visitors??0,visits:o.visits?.value??o.visits??0};return this.cache.set(e,g),g}async getSiteStats(){return this.getStats()}async getPageStats(t){return this.getStats(t)}clearCache(){this.cache.clear(),this.shareData=null,this.sharePromise=null}};function $(){window.oddmisc={getStats:()=>Promise.resolve({pageviews:0,visitors:0,visits:0}),getSiteStats:()=>Promise.resolve({pageviews:0,visitors:0,visits:0}),getPageStats:()=>Promise.resolve({pageviews:0,visitors:0,visits:0}),clearCache:()=>{}}}function b(r){if(!r.shareUrl){console.log("[oddmisc] shareUrl \u672A\u914D\u7F6E\uFF0C\u8DF3\u8FC7\u521D\u59CB\u5316"),$();return}try{let t=new C(r);window.oddmisc=window.oddmisc||{},window.oddmisc.umami=t,window.oddmisc.getStats=e=>t.getStats(e),window.oddmisc.getSiteStats=()=>t.getSiteStats(),window.oddmisc.getPageStats=e=>t.getPageStats(e),window.oddmisc.clearCache=()=>t.clearCache(),console.log("[oddmisc] Umami runtime client initialized")}catch(t){console.warn("[oddmisc] \u521D\u59CB\u5316\u5931\u8D25:",t instanceof Error?t.message:t),$()}}0&&(module.exports={CacheManager,UmamiClient,UmamiError,VERSION,createUmamiClient,initUmamiRuntime,isValidConfig,isValidShareUrl,parseShareUrl,umami});
